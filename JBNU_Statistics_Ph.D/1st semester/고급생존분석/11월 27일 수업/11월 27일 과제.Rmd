...
#### 변수 log(protime)을 포함한 Cox 모형 적합 후 패턴변화가 줄어듬.
...
#### 변수 age, log(protime)을 포함한 Cox 모형 적합 후 패턴변화가 줄어듬.
...
#### 변수 log(bili), log(protime)을 포함한 Cox 모형 적합 후 패턴변화가 줄어듬.
...
#### 변수 albumin, log(protime)을 포함한 Cox 모형 적합 후 패턴변화가 줄어듬.
...
#### 변수 log(protime), edema를 포함한 Cox 모형 적합 후 패턴변화가 줄어듬.
...
#### (1) ~ (7) 의 결과를 통해 사용 공변량을 비례위험모형에 포함시켰을 때 패턴변화가 줄어들고 있고 이산 공변량인 edema의 경우 비례성 관계를 만족하고 있다고 판단된다. 이에 따라 5가지 공변량을 모두 포함한 모형을 적합해보고 잔차분석을 실시하도록 한다.
...
fit <- coxph(Surv(time,censor)~age+log(bili)+albumin+log(protime)+edema,data=pbc)
summary(fit)
...
plot(age[is.na(log(protime))!=TRUE],r1,xlab="age",ylab="martingale residual")
lines(lowess(age[is.na(log(protime))!=TRUE],r1,iter=0),lty=2)
plot(age,r02,xlab="age",ylab="deviance residual")
lines(lowess(age,r02,iter=0),lty=2)
plot(age[is.na(log(protime))!=TRUE],r2,xlab="age",ylab="deviance residual")
lines(lowess(age[is.na(log(protime))!=TRUE],r2,iter=0),lty=2)
# log(bili) 
plot(log(bili),r01,xlab="log(bili)",ylab="martingale residual")
lines(lowess(log(bili),r01,iter=0),lty=2)
plot(log(bili)[is.na(log(protime))!=TRUE],r1,xlab="log(bili)",ylab="martingale residual")
lines(lowess(log(bili)[is.na(log(protime))!=TRUE],r1,iter=0),lty=2)
plot(log(bili),r02,xlab="log(bili)",ylab="deviance residual")
lines(lowess(log(bili),r02,iter=0),lty=2)
plot(log(bili)[is.na(log(protime))!=TRUE],r2,xlab="log(bili)",ylab="deviance residual")
lines(lowess(log(bili)[is.na(log(protime))!=TRUE],r2,iter=0),lty=2)
# albumin
plot(albumin,r01,xlab="albumin",ylab="martingale residual")
lines(lowess(albumin,r01,iter=0),lty=2)
plot(albumin[is.na(log(protime))!=TRUE],r1,xlab="albumin",ylab="martingale residual")
lines(lowess(albumin[is.na(log(protime))!=TRUE],r1,iter=0),lty=2)
plot(albumin,r02,xlab="albumin",ylab="deviance residual")
lines(lowess(albumin,r02,iter=0),lty=2)
plot(albumin[is.na(log(protime))!=TRUE],r2,xlab="albumin",ylab="deviance residual")
lines(lowess(albumin[is.na(log(protime))!=TRUE],r2,iter=0),lty=2)
# log(protime)
plot(log(protime)[is.na(log(protime))!=TRUE],r01[is.na(log(protime))!=TRUE],xlab="log(protime)",ylab="martingale residual")
lines(lowess(log(protime)[is.na(log(protime))!=TRUE],r01[is.na(log(protime))!=TRUE],iter=0),lty=2)
plot(log(protime)[is.na(log(protime))!=TRUE],r1,xlab="log(protime)",ylab="martingale residual")
lines(lowess(log(protime)[is.na(log(protime))!=TRUE],r1,iter=0),lty=2)
plot(log(protime)[is.na(log(protime))!=TRUE],r02[is.na(log(protime))!=TRUE],xlab="log(protime)",ylab="deviance residual")
lines(lowess(log(protime)[is.na(log(protime))!=TRUE],r02[is.na(log(protime))!=TRUE],iter=0),lty=2)
plot(log(protime)[is.na(log(protime))!=TRUE],r2,xlab="log(protime)",ylab="deviance residual")
lines(lowess(log(protime)[is.na(log(protime))!=TRUE],r2,iter=0),lty=2)
# edema
plot(edema,r01,xlab="edema",ylab="martingale residual")
lines(lowess(edema,r01,iter=0),lty=2)
plot(edema[is.na(log(protime))!=TRUE],r1,xlab="edema",ylab="martingale residual")
lines(lowess(edema[is.na(log(protime))!=TRUE],r1,iter=0),lty=2)
plot(edema,r02,xlab="edema",ylab="deviance residual")
lines(lowess(edema,r02,iter=0),lty=2)
plot(edema[is.na(log(protime))!=TRUE],r2,xlab="edema",ylab="deviance residual")
lines(lowess(edema[is.na(log(protime))!=TRUE],r2,iter=0),lty=2)
par(mfrow=c(1,1))
```

#### 모형적합 결과 모든 공변량이 유의하며, 5가지 공변량을 모두 포함했을 때 잔차의 패턴변화가 줄어듬을 확인할 수 있다.
...
summary(fit)
ggsurvplot(fit,data=pbc,conf.int=TRUE,risk.table.col="strata",ggtheme=theme_bw())
plot(fit,fun='cumhaz',col=c('red','green'),lty=c(1,2),xlab='Time',ylab='log Cumulative Hazard Rate')
legend(1500,0.6,legend=c('gtype=1','gtype=2'),lty=c(1,2),col=c('red','green'))
```
...
ggsurvplot(fit,data=pbc,conf.int=TRUE,risk.table.col="strata",ggtheme=theme_bw())
plot(fit,fun='cumhaz',col=c('red','green'),lty=c(1,2),xlab='Time',ylab='log Cumulative Hazard Rate')
legend(1500,0.6,legend=c('dtype=1','dtype=2'),lty=c(1,2),col=c('red','green'))
```
...
#### 분석결과 공변량이 없는 모형과 비교하였을 때 잔차의 선형성이 제거됨을을 확인할 수 있다.