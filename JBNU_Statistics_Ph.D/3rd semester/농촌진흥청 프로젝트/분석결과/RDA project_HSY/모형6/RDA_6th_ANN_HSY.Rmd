...
set.seed(1234)
ann1 <- neuralnet(내부CO2~외부기온+외부CO2+외부풍속+외부일사량+환기창면적, data=ex1_train, hidden=rep(5,1), threshold = 0.01, stepmax = 1e+08, act.fct='logistic')
summary(ann1)
# plot(ann1)
...
![](모형6 ANN 작물없음 첫번째 모형.png)

- RMSE=11.69821, SSE/SST=0.0000000151282
...
ex1_test <- as.data.frame(plantno_scale[-train.index,])
set.seed(1234)
ann1 <- neuralnet(내부CO2~외부기온+외부CO2+외부풍속+외부일사량+환기창면적, data=ex1_train, hidden=rep(3,2), threshold = 0.01, stepmax = 1e+08, act.fct='logistic')
summary(ann1)
# plot(ann1)
y <- (ex1_test$내부CO2)*(max(plantno$내부CO2)-min(plantno$내부CO2))+min(plantno$내부CO2)
y_hat_ann1 <- (compute(ann1, ex1_test)$net.result)*(max(plantno$내부CO2)-min(plantno$내부CO2))+min(plantno$내부CO2)
...
![](모형6 ANN 작물없음 두번째 모형.png)

- RMSE=11.71952, SSE/SST=0.00000001518337
...
![](모형6 ANN 작물있음 첫번째 모형.png)

- RMSE=4.140058, SSE/SST=0.000000008732776
- 적합결과 test RMSE와 SSE/SST의 값이 다중회귀모형과 비교했을 때 더 작은 값이므로 이 ANN 모형이 다중회귀모형보다 예측력이 좋다고 해석할 수 있다. 
...
![](모형6 ANN 작물있음 두번째 모형.png)

- RMSE=4.434662, SSE/SST=0.00000001001983
- 적합결과 test RMSE와 SSE/SST의 값이 다중회귀모형과 비교했을 때 더 작은 값이므로 이 ANN 모형이 다중회귀모형보다 예측력이 좋다고 해석할 수 있다. 첫번째 모형과 동일한 결과이다. 
