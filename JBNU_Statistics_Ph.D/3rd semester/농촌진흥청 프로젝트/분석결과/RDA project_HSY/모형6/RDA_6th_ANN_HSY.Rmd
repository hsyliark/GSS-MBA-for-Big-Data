...
```{r}
plantno_scale <- transform(plantno[,c(8,1,7,10,11,13)], 
                           내부CO2=(내부CO2-min(내부CO2))/(max(내부CO2)-min(내부CO2)),
...
- 첫번째로 고려한 적합모형은 hidden layer 5개, 각 layer에 대한 node 5개인 모형이다.
...
set.seed(1234)
ann1 <- neuralnet(내부CO2~외부기온+외부CO2+외부풍속+외부일사량+환기창면적, data=ex1_train, hidden=rep(5,5), threshold = 0.01, stepmax = 1e+08, act.fct='logistic')
summary(ann1)
# plot(ann1)
y <- (ex1_test$내부CO2)*(max(plantno$내부CO2)-min(plantno$내부CO2))+min(plantno$내부CO2)
y_hat_ann1 <- (compute(ann1, ex1_test)$net.result)*(max(plantno$내부CO2)-min(plantno$내부CO2))+min(plantno$내부CO2)
y_bar <- mean(y*(max(plantno$내부CO2)-min(plantno$내부CO2))+min(plantno$내부CO2))
...
- RMSE=11.53383, SSE/SST=0.00000001470603
- 적합결과 test RMSE와 SSE/SST의 값이 다중회귀모형과 비교했을 때 더 작은 값이므로 이 ANN 모형이 다중회귀모형보다 예측력이 좋다고 해석할 수 있다.

#### 두번째 모형

- 두번째로 고려한 적합모형은 hidden layer 10개, 각 layer에 대한 node 10개인 모형이다.
- activation function은 `logistic`으로 설정하였다.
...
ex1_test <- as.data.frame(plantno_scale[-train.index,])
set.seed(1234)
ann1 <- neuralnet(내부CO2~외부기온+외부CO2+외부풍속+외부일사량+환기창면적, data=ex1_train, hidden=rep(10,10), threshold = 0.01, stepmax = 1e+08, act.fct='logistic')
summary(ann1)
# plot(ann1)
y <- (ex1_test$내부CO2)*(max(plantno$내부CO2)-min(plantno$내부CO2))+min(plantno$내부CO2)
y_hat_ann1 <- (compute(ann1, ex1_test)$net.result)*(max(plantno$내부CO2)-min(plantno$내부CO2))+min(plantno$내부CO2)
...
- RMSE=12.275, SSE/SST=0.00000001665679
...
```{r}
plantyes_scale <- transform(plantyes[,c(8,1,7,10,11,13)], 
                           내부CO2=(내부CO2-min(내부CO2))/(max(내부CO2)-min(내부CO2)),
                           외부기온=(외부기온-min(외부기온))/(max(외부기온)-min(외부기온)),
                           외부CO2=(외부CO2-min(외부CO2))/(max(외부CO2)-min(외부CO2)),
                           외부풍속=(외부풍속-min(외부풍속))/(max(외부풍속)-min(외부풍속)),
...
- 첫번째로 고려한 적합모형은 hidden layer 2개, 각 layer에 대한 node 5개인 모형이다.
- activation function은 `logistic`으로 설정하였다.
...
set.seed(1234)
ann1 <- neuralnet(내부CO2~외부기온+외부CO2+외부풍속+외부일사량+환기창면적, data=ex1_train, hidden=rep(5,2), threshold = 0.01, stepmax = 1e+08, act.fct='logistic')
summary(ann1)
# plot(ann1)
y <- (ex1_test$내부CO2)*(max(plantyes$내부CO2)-min(plantyes$내부CO2))+min(plantyes$내부CO2)
y_hat_ann1 <- (compute(ann1, ex1_test)$net.result)*(max(plantyes$내부CO2)-min(plantyes$내부CO2))+min(plantyes$내부CO2)
y_bar <- mean(y*(max(plantyes$내부CO2)-min(plantyes$내부CO2))+min(plantyes$내부CO2))
RMSE <- sqrt(mean((y-y_hat_ann1)^2))
SSE <- sum((y-y_hat_ann1)^2) 
SST <- sum((y-y_bar)^2) 
result <- data.frame(y, y_hat_ann1, y-y_hat_ann1)
...
- RMSE=4.069568, SSE/SST=0.000000008437932
- 적합결과 test RMSE와 SSE/SST의 값이 다중회귀모형과 비교했을 때 더 작은 값이므로 이 ANN 모형이 다중회귀모형보다 예측력이 좋다고 해석할 수 있다. 
...
- RMSE=3.317978, SSE/SST=0.000000005609013
- 적합결과 test RMSE와 SSE/SST의 값이 다중회귀모형과 비교했을 때 더 작은 값이므로 이 ANN 모형이 다중회귀모형보다 예측력이 좋다고 해석할 수 있다. 첫번째 모형과 동일한 결과이다. 
