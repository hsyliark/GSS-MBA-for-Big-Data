...
- 분석결과 $R_{adj}^2$는 변수의 개수 13인 모형, $Cp$는 변수의 개수 8인 모형, 그리고 $BIC$는 변수의 개수 8인 모형을 최적으로 판단하고 있다. 이들 중 가장 간단한 변수의 개수 8인 모형을 선택하도록 한다. 복잡한 모형이 항상 바람직한 결과를 주지는 않으며 단순한 모형도 설명력이 높은 경우가 많기 때문이다.

```{r}
summary(lm(내외부CO2차~외부기온+외부CO2+환기창면적+외부기온*환기창면적+외부CO2*환기창면적+외부풍속*환기창면적+외부풍속*환기창면적+외부일사량*환기창면적,data=plantno))
```
...
# 모든 주효과와 가장 중요한 교호작용 1가지를 포함한 모형 (외부CO2와 환기창면적 사이의 교호작용)
...
```{r}
summary(lm(내외부CO2차~(외부기온+외부CO2+외부풍속+외부일사량+환기창면적)^2,data=plantno))
```
...
```{r}
summary(lm(내외부CO2차~외부기온+외부CO2+외부일사량+환기창면적+외부기온*외부CO2+외부기온*외부일사량+외부기온*환기창면적+외부CO2*외부일사량+외부CO2*환기창면적+외부풍속*환기창면적,data=plantyes))
```
...
- 모형을 적합한 결과 수치적인 문제로 인하여 예측값이 NA로 출력되는 경우가 발생함을 확인하였다. 이는 작물이 있는 경우 내외부CO2차에 대한 예측모형으로서 다중회귀모형이 그다지 바람직한 모형은 아닐수도 있음을 보여주는 것으로 보여진다. 다만 선택한 모형을 평가하기 위하여 예측값이 NA인 경우는 제외하고 척도를 계산하였다.
...
# 모든 주효과, 그리고 외부CO2와 외부일사량과의 교호작용 1개만을 포함한 모형
...
# 모든 주효과, 그리고 외부CO2와 외부기온과의 교호작용 1개만을 포함한 모형
...
- 결과적으로 5가지 모형들 중 모든 주효과, 그리고 외부CO2와 환기창면적과의 교호작용 1개만을 포함한 모형이 가장 우수하였다.

```{r}
summary(lm(내외부CO2차~외부기온+외부CO2+외부풍속+외부일사량+환기창면적+외부CO2*환기창면적,data=plantyes))
```
...
- 분석결과 $R_{adj}^2$는 변수의 개수 14인 모형, $Cp$는 변수의 개수 14인 모형, 그리고 $BIC$는 변수의 개수 12인 모형을 최적으로 판단하고 있다. 이에 따라 변수의 개수 12인 모형을 선택하도록 한다. 

```{r}
summary(lm(내외부CO2차~외부기온+외부CO2+외부일사량+환기창면적+작물유무+외부CO2*환기창면적+외부CO2*외부일사량+외부CO2*외부기온+작물유무*외부기온+작물유무*외부풍속+작물유무*외부일사량+작물유무*환기창면적,data=plant))
```
...
# 모든 주효과와 가장 중요한 교호작용 1가지(작물유무와 환기창면적)를 포함한 모형
...
# 모든 주효과와 가장 중요한 교호작용 2가지(작물유무와 환기창면적, 외부CO2와 환기창면적)를 포함한 모형
