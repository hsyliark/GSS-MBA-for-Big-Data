...
set.seed(1234)
ann1 <- neuralnet(내외부CO2차~외부기온+외부CO2+외부풍속+외부일사량+환기창면적, data=ex1_train, hidden=rep(5,2), threshold = 0.01, stepmax = 1e+08, act.fct='logistic')
summary(ann1)
# plot(ann1)
...
y_bar <- mean(y*(max(plantno$내외부CO2차)-min(plantno$내외부CO2차))+min(plantno$내외부CO2차))
...
- RMSE=11.19434, SSE/SST=0.000003951808
- 적합결과 test RMSE와 SSE/SST의 값이 다중회귀모형과 비교했을 때 더 작은 값이기는 하지만 거의 비슷하다. 이에 따라 일반적으로 ANN 모형이 다중회귀모형보다 예측력이 좋다고 해석할 수 있다.

#### 두번째 모형

- 두번째로 고려한 적합모형은 hidden layer 3개, 각 layer에 대한 node 4개인 모형이다.
- activation function은 `logistic`으로 설정하였다.
...
ex1_test <- as.data.frame(plantno_scale[-train.index,])
set.seed(1234)
ann1 <- neuralnet(내외부CO2차~외부기온+외부CO2+외부풍속+외부일사량+환기창면적, data=ex1_train, hidden=rep(4,3), threshold = 0.01, stepmax = 1e+08, act.fct='logistic')
summary(ann1)
# plot(ann1)
y <- (ex1_test$내외부CO2차)*(max(plantno$내외부CO2차)-min(plantno$내외부CO2차))+min(plantno$내외부CO2차)
y_hat_ann1 <- (compute(ann1, ex1_test)$net.result)*(max(plantno$내외부CO2차)-min(plantno$내외부CO2차))+min(plantno$내외부CO2차)
...
- RMSE=11.59102, SSE/SST=0.000004236839
- 적합결과 test RMSE와 SSE/SST의 값이 다중회귀모형과 비교했을 때 더 작은 값이기는 하지만 거의 비슷하다. 이에 따라 일반적으로 ANN 모형이 다중회귀모형보다 예측력이 좋다고 해석할 수 있다. 첫번째 모형과 동일한 결과이다.
...
y_bar <- mean(y*(max(plantyes$내외부CO2차)-min(plantyes$내외부CO2차))+min(plantyes$내외부CO2차))
RMSE <- sqrt(mean((y-y_hat_ann1)^2))
SSE <- sum((y-y_hat_ann1)^2) 
SST <- sum((y-y_bar)^2) 
result <- data.frame(y, y_hat_ann1, y-y_hat_ann1)
...
- RMSE=4.089194, SSE/SST=0.000002255476
- 적합결과 test RMSE와 SSE/SST의 값이 다중회귀모형과 비교했을 때 더 작은 값이므로 이 ANN 모형이 다중회귀모형보다 예측력이 좋다고 해석할 수 있다. 

#### 두번째 모형

- 두번째로 고려한 적합모형은 hidden layer 4개, 각 layer에 대한 node 3개인 모형이다.
- activation function은 `logistic`으로 설정하였다.
...
ex1_test <- as.data.frame(plantyes_scale[-train.index,])
set.seed(1234)
ann1 <- neuralnet(내외부CO2차~외부기온+외부CO2+외부풍속+외부일사량+환기창면적, data=ex1_train, hidden=rep(3,4), threshold = 0.01, stepmax = 1e+08, act.fct='logistic')
summary(ann1)
# plot(ann1)
y <- (ex1_test$내외부CO2차)*(max(plantyes$내외부CO2차)-min(plantyes$내외부CO2차))+min(plantyes$내외부CO2차)
y_hat_ann1 <- (compute(ann1, ex1_test)$net.result)*(max(plantyes$내외부CO2차)-min(plantyes$내외부CO2차))+min(plantyes$내외부CO2차)
...
- RMSE=5.814965, SSE/SST=0.000004560968
