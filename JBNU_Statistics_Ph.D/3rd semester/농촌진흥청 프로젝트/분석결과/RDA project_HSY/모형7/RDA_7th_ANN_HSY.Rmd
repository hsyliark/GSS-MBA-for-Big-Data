...
```{r}
plantno_scale <- transform(plantno[,c(9,12,1,7,10,11,13)], 
...
set.seed(1234)
ann1 <- neuralnet(내외부CO2차~외부기온+외부CO2+외부풍속+외부일사량+환기창면적, data=ex1_train, hidden=rep(20,20), threshold = 0.01, stepmax = 1e+08)
summary(ann1)
y_hat_ann1 <- compute(ann1, ex1_test)$net.result
RMSE <- mean(sum(((ex1_test$내외부CO2차-y_hat_ann1)*(max(plantno$내외부CO2차)-min(plantno$내외부CO2차))+min(plantno$내외부CO2차))^2))
RMSE
```

- 적합결과 test RMSE의 값이 71004.92로 산출되었다. 이는 다중회귀모형과 비교했을 때 좋지 않은 결과인데, 이미 구축한 다중회귀모형이 데이터를 충분히 잘 설명하도록 만들어졌기 때문이라고 생각한다.


### 3.2 작물이 있을 때

- 적합모형은 hidden layer 40개, 각 layer에 대한 node 30개인 모형이다.
- activation function은 `logistic`으로 설정하였다.

```{r}
plantyes_scale <- transform(plantyes[,c(9,12,1,7,10,11,13)], 
                           내외부CO2차=(내외부CO2차-min(내외부CO2차))/(max(내외부CO2차)-min(내외부CO2차)),
                           환기율=(환기율-min(환기율))/(max(환기율)-min(환기율)),
                           외부기온=(외부기온-min(외부기온))/(max(외부기온)-min(외부기온)),
                           외부CO2=(외부CO2-min(외부CO2))/(max(외부CO2)-min(외부CO2)),
...
set.seed(1234)
ann1 <- neuralnet(내외부CO2차~외부기온+외부CO2+외부풍속+외부일사량+환기창면적, data=ex1_train, hidden=rep(30,40), threshold = 0.01, stepmax = 1e+08)
summary(ann1)
y_hat_ann1 <- compute(ann1, ex1_test)$net.result
RMSE <- mean(sum(((ex1_test$내외부CO2차-y_hat_ann1)*(max(plantyes$내외부CO2차)-min(plantyes$내외부CO2차))+min(plantyes$내외부CO2차))^2))
RMSE
```

- 적합결과 test RMSE의 값이 117455.7로 산출되었다. 이는 다중회귀모형과 비교했을 때 좋지 않은 결과인데, 이미 구축한 다중회귀모형이 데이터를 충분히 잘 설명하도록 만들어졌기 때문이라고 생각한다.
