...
```{r}
plantno_scale <- transform(plantno[,c(2,1,10,11,13)], 
                           내부기온=(내부기온-min(내부기온))/(max(내부기온)-min(내부기온)),
...
set.seed(1234)
ann1 <- neuralnet(내부기온~외부기온+외부풍속+외부일사량+환기창면적, data=ex1_train, hidden=rep(10,3), threshold = 0.01, stepmax = 1e+08, act.fct='logistic')
summary(ann1)
# plot(ann1)
y <- (ex1_test$내부기온)*(max(plantno$내부기온)-min(plantno$내부기온))+min(plantno$내부기온)
y_hat_ann1 <- (compute(ann1, ex1_test)$net.result)*(max(plantno$내부기온)-min(plantno$내부기온))+min(plantno$내부기온)
y_bar <- mean(y*(max(plantno$내부기온)-min(plantno$내부기온))+min(plantno$내부기온))
...
- RMSE=0.8522163, SSE/SST=0.000001173895
...
ex1_test <- as.data.frame(plantno_scale[-train.index,])
set.seed(1234)
ann1 <- neuralnet(내부기온~외부기온+외부풍속+외부일사량+환기창면적, data=ex1_train, hidden=rep(7,5), threshold = 0.01, stepmax = 1e+08, act.fct='logistic')
summary(ann1)
# plot(ann1)
y <- (ex1_test$내부기온)*(max(plantno$내부기온)-min(plantno$내부기온))+min(plantno$내부기온)
y_hat_ann1 <- (compute(ann1, ex1_test)$net.result)*(max(plantno$내부기온)-min(plantno$내부기온))+min(plantno$내부기온)
...
- RMSE=0.8552394, SSE/SST=0.000001182238
...
```{r}
plantyes_scale <- transform(plantyes[,c(2,1,10,11,13)],                                                                           내부기온=(내부기온-min(내부기온))/(max(내부기온)-min(내부기온)),
                           외부기온=(외부기온-min(외부기온))/(max(외부기온)-min(외부기온)),
                           외부풍속=(외부풍속-min(외부풍속))/(max(외부풍속)-min(외부풍속)),
                           외부일사량=(외부일사량-min(외부일사량))/(max(외부일사량)-min(외부일사량)),
                           환기창면적=(환기창면적-min(환기창면적))/(max(환기창면적)-min(환기창면적)))
...
set.seed(1234)
ann1 <- neuralnet(내부기온~외부기온+외부풍속+외부일사량+환기창면적, data=ex1_train, hidden=rep(6,2), threshold = 0.01, stepmax = 1e+08)
summary(ann1)
# plot(ann1)
y <- (ex1_test$내부기온)*(max(plantyes$내부기온)-min(plantyes$내부기온))+min(plantyes$내부기온)
y_hat_ann1 <- (compute(ann1, ex1_test)$net.result)*(max(plantyes$내부기온)-min(plantyes$내부기온))+min(plantyes$내부기온)
y_bar <- mean(y*(max(plantyes$내부기온)-min(plantyes$내부기온))+min(plantyes$내부기온))
RMSE <- sqrt(mean((y-y_hat_ann1)^2))
SSE <- sum((y-y_hat_ann1)^2) 
SST <- sum((y-y_bar)^2) 
result <- data.frame(y, y_hat_ann1, y-y_hat_ann1)
...
- RMSE=1.127848, SSE/SST=0.000001360429
- 적합결과 test RMSE와 SSE/SST의 값이 다중회귀모형과 비교했을 때 더 작은 값이므로 이 ANN 모형이 다중회귀모형보다 예측력이 좋다고 해석할 수 있다. 
...
ex1_test <- as.data.frame(plantyes_scale[-train.index,])
set.seed(1234)
ann1 <- neuralnet(내부기온~외부기온+외부풍속+외부일사량+환기창면적, data=ex1_train, hidden=rep(5,5), threshold = 0.01, stepmax = 1e+08)
summary(ann1)
# plot(ann1)
y <- (ex1_test$내부기온)*(max(plantyes$내부기온)-min(plantyes$내부기온))+min(plantyes$내부기온)
y_hat_ann1 <- (compute(ann1, ex1_test)$net.result)*(max(plantyes$내부기온)-min(plantyes$내부기온))+min(plantyes$내부기온)
...
- RMSE=1.113997, SSE/SST=0.000001327219
- 적합결과 test RMSE와 SSE/SST의 값이 다중회귀모형과 비교했을 때 더 작은 값이므로 이 ANN 모형이 다중회귀모형보다 예측력이 좋다고 해석할 수 있다. 첫번째 모형과 동일한 결과이다.
