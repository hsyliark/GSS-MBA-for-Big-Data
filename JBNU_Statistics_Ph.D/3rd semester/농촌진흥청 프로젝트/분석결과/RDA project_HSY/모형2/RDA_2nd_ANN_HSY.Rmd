...
- 첫번째로 고려한 적합모형은 hidden layer 1개, 각 layer에 대한 node 4개인 모형이다.
...
set.seed(1234)
ann1 <- neuralnet(내부기온~외부기온+외부풍속+외부일사량+환기창면적, data=ex1_train, hidden=rep(4,1), threshold = 0.01, stepmax = 1e+08, act.fct='logistic')
summary(ann1)
# plot(ann1)
...
![](모형2 ANN 작물없음 첫번째 모형.png)

- RMSE=0.863062, SSE/SST=0.000001203964
- 적합결과 test RMSE와 SSE/SST의 값이 다중회귀모형과 비교했을 때 더 작은 값이므로 이 ANN 모형이 다중회귀모형보다 예측력이 좋다고 해석할 수 있다.

#### 두번째 모형

- 두번째로 고려한 적합모형은 hidden layer 2개, 각 layer에 대한 node 2개인 모형이다.
- activation function은 `logistic`으로 설정하였다.
...
ex1_test <- as.data.frame(plantno_scale[-train.index,])
set.seed(1234)
ann1 <- neuralnet(내부기온~외부기온+외부풍속+외부일사량+환기창면적, data=ex1_train, hidden=rep(2,2), threshold = 0.01, stepmax = 1e+08, act.fct='logistic')
summary(ann1)
# plot(ann1)
y <- (ex1_test$내부기온)*(max(plantno$내부기온)-min(plantno$내부기온))+min(plantno$내부기온)
y_hat_ann1 <- (compute(ann1, ex1_test)$net.result)*(max(plantno$내부기온)-min(plantno$내부기온))+min(plantno$내부기온)
...
![](모형2 ANN 작물없음 두번째 모형.png)

- RMSE=1.001796, SSE/SST=0.000001622141
...
set.seed(1234)
ann1 <- neuralnet(내부기온~외부기온+외부풍속+외부일사량+환기창면적, data=ex1_train, hidden=rep(4,1), threshold = 0.01, stepmax = 1e+08)
summary(ann1)
# plot(ann1)
...
![](모형2 ANN 작물있음 첫번째 모형.png)

- RMSE=1.17249, SSE/SST=0.000001470254
- 적합결과 test RMSE와 SSE/SST의 값이 다중회귀모형과 비교했을 때 더 작은 값이므로 이 ANN 모형이 다중회귀모형보다 예측력이 좋다고 해석할 수 있다. 
...
ex1_test <- as.data.frame(plantyes_scale[-train.index,])
set.seed(1234)
ann1 <- neuralnet(내부기온~외부기온+외부풍속+외부일사량+환기창면적, data=ex1_train, hidden=rep(2,2), threshold = 0.01, stepmax = 1e+08)
summary(ann1)
# plot(ann1)
y <- (ex1_test$내부기온)*(max(plantyes$내부기온)-min(plantyes$내부기온))+min(plantyes$내부기온)
y_hat_ann1 <- (compute(ann1, ex1_test)$net.result)*(max(plantyes$내부기온)-min(plantyes$내부기온))+min(plantyes$내부기온)
...
![](모형2 ANN 작물있음 두번째 모형.png)

- RMSE=1.282065, SSE/SST=0.000001757902
- 적합결과 test RMSE와 SSE/SST의 값이 다중회귀모형과 비교했을 때 더 작은 값이므로 이 ANN 모형이 다중회귀모형보다 예측력이 좋다고 해석할 수 있다. 첫번째 모형과 동일한 결과이다.
