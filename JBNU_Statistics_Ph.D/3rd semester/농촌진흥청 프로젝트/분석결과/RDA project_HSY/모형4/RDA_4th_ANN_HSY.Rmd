...
set.seed(1234)
ann1 <- neuralnet(내부상대습도~외부기온+외부상대습도+외부풍속+외부일사량+환기창면적, data=ex1_train, hidden=rep(5,1), threshold = 0.01, stepmax = 1e+08, act.fct='logistic')
summary(ann1)
# plot(ann1)
...
![](모형4 ANN 작물없음 첫번째 모형.png)

- RMSE=1.350816, SSE/SST=0.000001641859
...
ex1_test <- as.data.frame(plantno_scale[-train.index,])
set.seed(1234)
ann1 <- neuralnet(내부상대습도~외부기온+외부상대습도+외부풍속+외부일사량+환기창면적, data=ex1_train, hidden=rep(4,2), threshold = 0.01, stepmax = 1e+08, act.fct='logistic')
summary(ann1)
# plot(ann1)
y <- (ex1_test$내부상대습도)*(max(plantno$내부상대습도)-min(plantno$내부상대습도))+min(plantno$내부상대습도)
y_hat_ann1 <- (compute(ann1, ex1_test)$net.result)*(max(plantno$내부상대습도)-min(plantno$내부상대습도))+min(plantno$내부상대습도)
...
![](모형4 ANN 작물없음 두번째 모형.png)

- RMSE=1.202883, SSE/SST=0.000001301938
...
![](모형4 ANN 작물있음 첫번째 모형.png)

- RMSE=2.590894, SSE/SST=0.0000008059027
- 적합결과 test RMSE와 SSE/SST의 값이 다중회귀모형과 비교했을 때 약간 크지만 거의 비슷하다. 그러므로 전체적으로 판단한다면 ANN 모형이 다중회귀모형보다 일반적으로 예측력이 좋다고 할 수 있겠다.
...
![](모형4 ANN 작물있음 두번째 모형.png)

- RMSE=2.601448, SSE/SST=0.0000008124818
