...
- 그림을 통해서 봤을때 환기창면적과 다른 설명변수들 간의 뚜렷한 교호작용은 없는 것으로 보인다.
- 산점도를 통해 알 수 있는 사실은 외부풍속이 증가갈수록, 외부기온과 외부일사량이 증가할수록 환기율이 올라간다는 점이다.  
...
```{r}
m_no <- lm(환기율~(외부기온+외부풍속+외부일사량+환기창면적)^2,data=plantno) # 다중회귀모형 구축 함수
summary(m_no)
```

- 모형적합 결과 외부기온, 그리고 외부기온과 환기창면적 사이의 교호작용이 유의수준 0.05에서 유의미하지 않다.
- 그리고 교호작용들 중 외부풍속과 환기창면적 사이의 교호작용에 대한 t 통계량의 절대값이 가장 크므로 이를 가장 중요한 교호작용으로 결정한다.
- 조정된 결정계수의 값이 $R^2_{adj}=0.9965$이다.
...
ms0 <- regsubsets(환기율~(외부기온+외부풍속+외부일사량+환기창면적)^2,data=plantno,nvmax=10) 
...
- 분석결과 $R_{adj}^2$와 $Cp$는 변수의 개수 9인 모형, 그리고 $BIC$는 변수의 개수 7인 모형을 최적으로 판단하고 있다. 이에 따라 변수의 개수 7인 모형을 선택하도록 한다.

```{r}
summary(lm(환기율~외부일사량+환기창면적+외부기온*외부풍속+외부기온*외부일사량+외부풍속*외부일사량+외부풍속*환기창면적+외부일사량*환기창면적,data=plantno))
```
...
m0 <- lm(환기율~(외부기온+외부풍속+외부일사량+환기창면적)^2,data=ex1_train) 
...
m0 <- lm(환기율~외부일사량+환기창면적+외부기온*외부풍속+외부기온*외부일사량+외부풍속*외부일사량+외부풍속*환기창면적+외부일사량*환기창면적,data=ex1_train) 
y_hat0 <- predict(m0,ex1_test)
...
- 결과적으로 3가지 모형들 중 Adj R^2, Cp, BIC 등의 기준을 바탕으로 선택된 모형의 성능이 가장 우수하였다.
- `:`표시는 교호작용을 의미함.

- model1 : 모든 주효과와 교호작용을 포함한 모형 
    - 반응변수 : 환기율
    - 설명변수 : 외부기온, 외부풍속, 외부일사량, 환기창면적, 외부기온:외부풍속, 외부기온:외부일사량, 외부기온:환기창면적, 외부풍속:외부일사량, 외부풍속:환기창면적, 외부일사량:환기창면적
    - RMSE=0.0977, SSE/SST=0.0036
    
- model2 : Adj R^2, Cp, BIC 등의 기준을 바탕으로 선택된 모형 (교호작용이 있는 경우는 주효과 포함) 
    - 반응변수 : 환기율
    - 설명변수 : 외부일사량, 환기창면적, 외부기온:외부풍속, 외부기온:외부일사량, 외부풍속:외부일사량, 외부풍속:환기창면적, 외부일사량:환기창면적
    - RMSE=0.0977, SSE/SST=0.0036
...
```{r}
library(leaps)
ms0 <- regsubsets(환기율~(외부기온+외부풍속+외부일사량+환기창면적)^2,data=plantyes,nvmax=10)
ms0.summary <- summary(ms0)
ms0.summary
...
- 분석결과 $R_{adj}^2$, $Cp$, 그리고 $BIC$ 모두 변수의 개수 9인 모형을 최적으로 판단하고 있다. 이에 따라 변수의 개수 9인 모형을 선택하도록 한다. 

```{r}
summary(lm(환기율~외부기온+외부풍속+외부일사량+환기창면적+외부기온*외부일사량+외부기온*환기창면적+외부풍속*외부일사량+외부풍속*환기창면적+외부일사량*환기창면적,data=plantyes))
```
...
m0 <- lm(환기율~외부기온+외부풍속+외부일사량+환기창면적+외부기온*외부일사량+외부기온*환기창면적+외부풍속*외부일사량+외부풍속*환기창면적+외부일사량*환기창면적,data=ex1_train) 
y_hat0 <- predict(m0,ex1_test)
RMSE1 <- sqrt(mean((ex1_test$환기율-y_hat0)^2))
SSE1 <- sum((ex1_test$환기율-y_hat0)^2) 
SST1 <- sum((ex1_test$환기율-mean(ex1_test$환기율))^2) 
...
m0 <- lm(환기율~외부기온+외부풍속+외부일사량+환기창면적+외부풍속*환기창면적+외부풍속*외부일사량,data=ex1_train) 
y_hat0 <- predict(m0,ex1_test)
RMSE2 <- sqrt(mean((ex1_test$환기율-y_hat0)^2))
SSE2 <- sum((ex1_test$환기율-y_hat0)^2) 
SST2 <- sum((ex1_test$환기율-mean(ex1_test$환기율))^2) 
...
# model4 : 모든 주효과, 그리고 외부풍속과 환기창면적 사이의 교호작용 1개만을 포함한 모형
m0 <- lm(환기율~외부기온+외부풍속+외부일사량+환기창면적+외부풍속*환기창면적,data=ex1_train) 
y_hat0 <- predict(m0,ex1_test)
RMSE3 <- sqrt(mean((ex1_test$환기율-y_hat0)^2))
SSE3 <- sum((ex1_test$환기율-y_hat0)^2) 
SST3 <- sum((ex1_test$환기율-mean(ex1_test$환기율))^2) 
result3 <- data.frame(ex1_test$환기율, y_hat0, ex1_test$환기율-y_hat0)
colnames(result3) <- c('y','yhat','y-yhat')
RMSE3 # test RMSE
SSE3/SST3 # SSE/SST
head(result3,12)

# model5 : 모든 주효과, 그리고 외부풍속과 외부일사량 사이의 교호작용 1개만을 포함한 모형
m0 <- lm(환기율~외부기온+외부풍속+외부일사량+환기창면적+외부풍속*외부일사량,data=ex1_train) 
y_hat0 <- predict(m0,ex1_test)
RMSE4 <- sqrt(mean((ex1_test$환기율-y_hat0)^2))
SSE4 <- sum((ex1_test$환기율-y_hat0)^2) 
SST4 <- sum((ex1_test$환기율-mean(ex1_test$환기율))^2) 
result4 <- data.frame(ex1_test$환기율, y_hat0, ex1_test$환기율-y_hat0)
...
- 결과적으로 Adj R^2, Cp, BIC에 의하여 선택된 모형이 가장 우수하였다.
- `:`표시는 교호작용을 의미함.

- model1 : 모든 주효과 및 교호작용을 포함한 모형 
    - 반응변수 : 환기율
    - 설명변수 : 외부기온, 외부풍속, 외부일사량, 환기창면적, 외부기온:외부풍속, 외부기온:외부일사량, 외부기온:환기창면적, 외부풍속:외부일사량, 외부풍속:환기창면적, 외부일사량:환기창면적
    - RMSE=0.3207, SSE/SST=0.0167
    
- model2 : Adj R^2, Cp, BIC에 의하여 선택된 모형 
    - 반응변수 : 환기율
    - 설명변수 : 외부기온, 외부풍속, 외부일사량, 환기창면적, 외부기온:외부일사량, 외부기온:환기창면적, 외부풍속:외부일사량, 외부풍속:환기창면적, 외부일사량:환기창면적
    - RMSE=0.3133, SSE/SST=0.0160
    
- model3 : 모든 주효과, 그리고 중요한 2개의 교호작용을 포함한 모형
    - 반응변수 : 환기율
    - 설명변수 : 외부기온, 외부풍속, 외부일사량, 환기창면적, 외부풍속:환기창면적, 외부풍속:외부일사량
    - RMSE=0.3368, SSE/SST=0.0185  
...
- model5 : 모든 주효과, 그리고 외부풍속과 외부일사량 사이의 교호작용 1개만을 포함한 모형
    - 반응변수 : 환기율
    - 설명변수 : 외부기온, 외부풍속, 외부일사량, 환기창면적, 외부풍속:외부일사량
    - RMSE=0.5632, SSE/SST=0.0517    
...
- 2.1절과 2.2절의 결과를 통해 외부풍속과 환기창면적과의 교호작용이 가장 유의미하다는 사실을 파악할 수 있다.
- 여기에서는 작물유무에 대한 변수가 추가로 들어가고 이에 따라 작물유무에 대한 교호작용을 포함해서 다양한 교호작용 변수를 고려할 수 있지만 모형이 너무 복잡해지므로 바람직하지 않을 수 있다. 
- 따라서 환기율을 반응변수로 하여 각 변수의 주효과, 그리고 외부풍속과 환기창면적 사이의 교호작용을 포함한 모형을 시작점으로 하여 분석을 진행하도록 한다. 여기에 주효과와 작물유무 사이의 교호작용도 우선 포함한다. 
...
- 모형적합 결과 외부기온이 유의수준 0.05에서 유의미하지 않다. 
- 그리고 가장 중요한 교호작용은 두가지를 선택할 수 있는데 환기창면적과 작물유무, 그리고 외부풍속과 환기창면적 사이의 교호작용이라고 판단된다. 
- 조정된 결정계수의 값이 $R^2_{adj}=0.9846$이다.
...
```{r}
summary(lm(환기율~외부풍속+외부일사량+환기창면적+작물유무+외부기온*작물유무+외부풍속*작물유무+외부일사량*작물유무+환기창면적*작물유무+외부풍속*환기창면적,data=plant))
```
...
# model2 : Adj R^2, Cp, BIC 등의 기준을 바탕으로 선택된 모형 
m0 <- lm(환기율~외부풍속+외부일사량+환기창면적+작물유무+외부기온*작물유무+외부풍속*작물유무+외부일사량*작물유무+환기창면적*작물유무+외부풍속*환기창면적,data=ex1_train)
y_hat0 <- predict(m0,ex1_test)
RMSE1 <- sqrt(mean((ex1_test$환기율-y_hat0)^2))
SSE1 <- sum((ex1_test$환기율-y_hat0)^2) 
SST1 <- sum((ex1_test$환기율-mean(ex1_test$환기율))^2) 
...
# model3 : 모든 주효과와 가장 중요한 교호작용 1가지(환기창면적과 작물유무)를 포함한 모형
m0 <- lm(환기율~외부기온+외부풍속+외부일사량+환기창면적+작물유무+환기창면적*작물유무,data=ex1_train)
y_hat0 <- predict(m0,ex1_test)
RMSE2 <- sqrt(mean((ex1_test$환기율-y_hat0)^2))
SSE2 <- sum((ex1_test$환기율-y_hat0)^2) 
SST2 <- sum((ex1_test$환기율-mean(ex1_test$환기율))^2) 
...
# model4 : 모든 주효과와 가장 중요한 교호작용 2가지(환기창면적과 작물유무, 외부풍속과 환기창면적)를 포함한 모형
m0 <- lm(환기율~외부기온+외부풍속+외부일사량+환기창면적+작물유무+환기창면적*작물유무+외부풍속*환기창면적,data=ex1_train)
y_hat0 <- predict(m0,ex1_test)
RMSE3 <- sqrt(mean((ex1_test$환기율-y_hat0)^2))
SSE3 <- sum((ex1_test$환기율-y_hat0)^2) 
SST3 <- sum((ex1_test$환기율-mean(ex1_test$환기율))^2) 
...
- Adj R^2, Cp, BIC 등의 기준을 바탕으로 선택된 모형도 사실은 외부기온을 제외하는 모형이지만 다른 설명변수와의 교호작용이 유의미한 경우가 있기 때문에 결과적으로는 모형에 포함된다. 따라서 최초로 고려한 모형과 같은 모형이다.
- `:`표시는 교호작용을 의미함.
...
- model2 : Adj R^2, Cp, BIC 등의 기준을 바탕으로 선택된 모형 
    - 반응변수 : 환기율
    - 설명변수 : 외부풍속, 외부일사량, 환기창면적, 작물유무, 외부기온:작물유무, 외부풍속:작물유무, 외부일사량:작물유무, 환기창면적:작물유무, 환기창면적:외부풍속
    - RMSE=0.2840, SSE/SST=0.0132
    
- model3 : 모든 주효과와 가장 중요한 교호작용 1가지(환기창면적과 작물유무)를 포함한 모형 
    - 반응변수 : 환기율
    - 설명변수 : 외부기온, 외부풍속, 외부일사량, 환기창면적, 작물유무, 환기창면적:작물유무
    - RMSE=0.7270, SSE/SST=0.0866  
    
- model4 : 모든 주효과와 가장 중요한 교호작용 2가지(환기창면적과 작물유무, 외부풍속과 환기창면적) 포함한 모형
    - 반응변수 : 환기율
    - 설명변수 : 외부기온, 외부풍속, 외부일사량, 환기창면적, 작물유무, 환기창면적:작물유무, 외부풍속:환기창면적
    - RMSE=0.4113, SSE/SST=0.0277    
