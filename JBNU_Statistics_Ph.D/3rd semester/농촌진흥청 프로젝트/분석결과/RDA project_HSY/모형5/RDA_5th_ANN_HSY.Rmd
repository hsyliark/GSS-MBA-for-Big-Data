...
```{r}
plantno_scale <- transform(plantno[,c(6,12,1,4,10,11,13)], 
...
set.seed(1234)
ann1 <- neuralnet(내외부상대습도차~외부기온+외부상대습도+외부풍속+외부일사량+환기창면적, data=ex1_train, hidden=rep(10,20), threshold = 0.01, stepmax = 1e+08)
summary(ann1)
y_hat_ann1 <- compute(ann1, ex1_test)$net.result
RMSE <- mean(sum(((ex1_test$내외부상대습도차-y_hat_ann1)*(max(plantno$내외부상대습도차)-min(plantno$내외부상대습도차))+min(plantno$내외부상대습도차))^2))
RMSE
```

- 적합결과 test RMSE의 값이 6897.343으로 산출되었다. 이는 다중회귀모형과 비교했을 때 좋지 않은 결과인데, 이미 구축한 다중회귀모형이 데이터를 충분히 잘 설명하도록 만들어졌기 때문이라고 생각한다.
...
```{r}
plantyes_scale <- transform(plantyes[,c(6,12,1,4,10,11,13)], 
                           내외부상대습도차=(내외부상대습도차-min(내외부상대습도차))/(max(내외부상대습도차)-min(내외부상대습도차)),
                           환기율=(환기율-min(환기율))/(max(환기율)-min(환기율)),
                           외부기온=(외부기온-min(외부기온))/(max(외부기온)-min(외부기온)),
                           외부상대습도=(외부상대습도-min(외부상대습도))/(max(외부상대습도)-min(외부상대습도)),
...
set.seed(1234)
ann1 <- neuralnet(내외부상대습도차~외부기온+외부상대습도+외부풍속+외부일사량+환기창면적, data=ex1_train, hidden=rep(20,20), threshold = 0.01, stepmax = 1e+08)
summary(ann1)
y_hat_ann1 <- compute(ann1, ex1_test)$net.result
RMSE <- mean(sum(((ex1_test$내외부상대습도차-y_hat_ann1)*(max(plantyes$내외부상대습도차)-min(plantyes$내외부상대습도차))+min(plantyes$내외부상대습도차))^2))
RMSE
```

- 적합결과 test RMSE의 값이 15653.89로 산출되었다. 이는 다중회귀모형과 비교했을 때 좋지 않은 결과인데, 이미 구축한 다중회귀모형이 데이터를 충분히 잘 설명하도록 만들어졌기 때문이라고 생각한다.
