...
set.seed(1234)
ann1 <- neuralnet(내외부상대습도차~외부기온+외부상대습도+외부풍속+외부일사량+환기창면적, data=ex1_train, hidden=rep(6,1), threshold = 0.01, stepmax = 1e+08, act.fct='logistic')
summary(ann1)
# plot(ann1)
...
![](모형5 ANN 작물없음 첫번째 모형.png)

- RMSE=1.113862, SSE/SST=0.00001934321
- 적합결과 test RMSE와 SSE/SST의 값이 다중회귀모형과 비교했을 때 더 작은 값이므로 이 ANN 모형이 다중회귀모형보다 예측력이 좋다고 해석할 수 있다. 

#### 두번째 모형

- 두번째로 고려한 적합모형은 hidden layer 2개, 각 layer에 대한 node 5개인 모형이다.
- activation function은 `logistic`으로 설정하였다.
...
ex1_test <- as.data.frame(plantno_scale[-train.index,])
set.seed(1234)
ann1 <- neuralnet(내외부상대습도차~외부기온+외부상대습도+외부풍속+외부일사량+환기창면적, data=ex1_train, hidden=rep(5,2), threshold = 0.01, stepmax = 1e+08, act.fct='logistic')
summary(ann1)
# plot(ann1)
y <- (ex1_test$내외부상대습도차)*(max(plantno$내외부상대습도차)-min(plantno$내외부상대습도차))+min(plantno$내외부상대습도차)
y_hat_ann1 <- (compute(ann1, ex1_test)$net.result)*(max(plantno$내외부상대습도차)-min(plantno$내외부상대습도차))+min(plantno$내외부상대습도차)
...
![](모형5 ANN 작물없음 두번째 모형.png)

- RMSE=1.011524, SSE/SST=0.00001595214
...
![](모형5 ANN 작물있음 첫번째 모형.png)

- RMSE=2.414533, SSE/SST=0.00001831324
- 적합결과 test RMSE와 SSE/SST의 값이 다중회귀모형과 비교했을 때 더 작은 값이므로 이 ANN 모형이 다중회귀모형보다 예측력이 좋다고 해석할 수 있다.
...
![](모형5 ANN 작물있음 두번째 모형.png)

- RMSE=2.509176, SSE/SST=0.00001977703
