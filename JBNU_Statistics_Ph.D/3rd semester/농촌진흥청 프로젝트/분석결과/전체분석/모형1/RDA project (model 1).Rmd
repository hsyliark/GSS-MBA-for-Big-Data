...
- 하지만 이 결과는 모형적합 후 단순히 유의한 영향력이 있는지를 확인한 것에 불과하다. 그리고 산점도행렬을 토대로 판단했을 때 환기창면적과 외부풍속 사이에도 교호작용이 존재하는 것으로 판단된다. 이에 따라 best subset selection 방법을 통해 모형을 다시 선택해볼 것이며, 최종적으로 선택된 모형에 대해 환기창면적과 외부풍속 간의 교호작용을 추가하여 모형을 적합해볼 것이다. 이후에는 train:test=7:3 으로 데이터를 나누어서 4개의 교호작용 term을 하나씩 제외한 모형들과 모두 포함한 모형, 그리고 최종적으로 선택된 모형에 대해 환기창면적과 외부풍속 간의 교호작용까지 추가한 모형을 test RMSE의 관점에서 비교해볼 것이다.
...
- ${\hat{y}} = -2.2977 + 0.0002x_{1} + 0.7685x_{2} + 0.0004x_{3} + 30.2437x_{4} - 1.1729x_{5} - 0.0412x_{1}x_{5} + 1.2095x_{2}x_{5} + 0.0011x_{3}x_{5} + 14.9740x_{4}x_{5}$
- $y$ : 환기율, $x _{1}$ : 외부기온, $x _{2}$ : 외부풍속, $x_{3}$ : 외부일사량, $x_{4}$ : 환기창면적, $x _{5}$ : 작물유무yes
- 추가로 이 모형에 환기창면적과 외부풍속 간의 교호작용을 추가한 모형을 적합하면 다음과 같다.

```{r}
ms0.res_plus <- lm(환기율~외부기온+외부풍속+외부일사량+환기창면적+작물유무+
                   외부기온*작물유무+외부풍속*작물유무+
                   외부일사량*작물유무+환기창면적*작물유무+환기창면적*외부풍속,
                data=ex1)
summary(ms0.res_plus)
```

- 분석결과, 환기창면적과 외부풍속 사이에도 명백한 교호작용이 있다는 결론을 주고 있으며 모형식은 다음과 같다.
- ${\hat{y}} = 0.2226 + 0.0002x_{1} - 0.1620x_{2} + 0.0004x_{3} + 3.2410x_{4} - 1.9340x_{5} - 0.0201x_{1}x_{5} + 0.7009x_{2}x_{5} + 0.0012x_{3}x_{5} + 25.1200x_{4}x_{5} + 9.9690x_{2}x_{4}$
- $y$ : 환기율, $x _{1}$ : 외부기온, $x _{2}$ : 외부풍속, $x_{3}$ : 외부일사량, $x_{4}$ : 환기창면적, $x _{5}$ : 작물유무yes


### train:test

- 지금부터는 train:test=7:3 으로 데이터를 나누어서 4개의 교호작용 term을 하나씩 제외한 모형들과 모두 포함한 모형, 그리고 최종적으로 선택된 모형에 대해 환기창면적과 외부풍속 간의 교호작용까지 추가한 모형을 test RMSE의 관점에서 비교해볼 것이다. test RMSE가 작을수록 예측을 잘하는 모형이라고 해석하면 된다.
...
RMSE4
m5 <- lm(환기율~외부기온+외부풍속+외부일사량+환기창면적+작물유무+
              외부기온*작물유무+외부풍속*작물유무+
              외부일사량*작물유무+환기창면적*작물유무+환기창면적*외부풍속,
            data=ex1_train) # 모든 변수 포함 + 환기창면적*외부풍속
y_hat5 <- predict(m5,ex1_test)
RMSE5 <- mean(sum((ex1_test$환기율-y_hat5)^2))
RMSE5
```

- 결과적으로 6가지 모형들 중 환기창면적과 외부풍속의 교호작용까지 추가한 모형에 대한 test RMSE의 값이 가장 작음을 확인할 수 있다. 결론적으로 best subset selection을 통해 선정된 모형과 그 결과에 환기창면적과 외부풍속의 교호작용까지 추가한 모형에 대한 결과를 정리하면 다음과 같다.

```{r}
summary(m0) # Best subset selection 결과
```
...
```{r}
summary(m5) # 환기창면적*외부풍속 추가
```

- ${\hat{y}} = 0.3128 - 0.0009x_{1} - 0.1858x_{2} + 0.0004x_{3} + 2.5790x_{4} - 1.9290x_{5} - 0.0207x_{1}x_{5} + 0.7079x_{2}x_{5} + 0.0012x_{3}x_{5} + 25.2300x_{4}x_{5} + 10.2100x_{2}x_{4}$
- $y$ : 환기율, $x _{1}$ : 외부기온, $x _{2}$ : 외부풍속, $x_{3}$ : 외부일사량, $x_{4}$ : 환기창면적, $x _{5}$ : 작물유무yes


### residual analysis

- 최종적으로 선택한 모형이 회귀분석에 대한 가정을 만족하고 있는지 잔차분석을 실시해보도록 한다.
...
- 위에 있는 잔차와 관련한 4가지 그림을 보면 우선 잔차의 흩어진 정도가 균일하지 않고 약간 곡선형태의 움직임을 보이고 있다. 그렇기 때문에 이 모형에 대해 적절한 변환함수를 적용하여 변수변환을 실시한다면 더 좋은 결과를 도출할 수도 있을 것이라고 생각한다.
- 그리고 눈에 띄는 이상치가 3개 정도 있는 것으로 보여진다. 
- 또한 Q-Q plot을 통해 잔차가 정규성을 만족하고 있지는 않는 것 같다. 이에 대해 아래와 같이 Shapiro-Wilk normality test를 해볼 수 있다.

```{r}
shapiro.test(m5$residuals)
```

- 결과적으로 유의확률이 유의수준 $\alpha=0.05$보다 매우 작은 값이기 때문에 잔차가 정규성을 위배하고 있다고 말할 수 있다.
- 추가로 잔차가 자기상관성을 가지고 있는지 확인하기 위해 Durbin-Watson test를 다음과 같이 실시할 수 있다. 

```{r}
library(lmtest)
dwtest(m5,alternative='greater')
dwtest(m5,alternative='less')
dwtest(m5,alternative='two.sided')
```

- 위의 결과를 통해 양측검정, 단측검정 모두 유의확률이 유의수준 $\alpha=0.05$보다 크기 때문에 잔차에 대한 자기상관성이 존재하지 않는다고 말할 수 있다.
...
- 결과적으로 train data를 이용하여 ANN 모형을 적합하고 이를 이용하여 test data에 대해 계산한 RMSE의 값은 $RMSE=52.31884$이다.
