...
- 분석결과, pearson 표본상관계수에 따르면 내부기온과 환기율, 내부기온과 외부풍속, 내부기온과 환기창면적, 환기율과 외부일사량, 외부풍속과 환기창면적, 외부일사량과 환기창면적은 음의 상관관계를 보이고 있으며, 나머지는 모두 양의 상관관계를 보이고 있다. 이를 그림으로 그리면 다음과 같다.
...
```{r}
m0 <- lm(내부기온~환기율+외부기온+외부풍속+외부일사량+환기창면적+작물유무+
...
- 모형적합 결과 환기율, 외부기온, 외부풍속, 외부일사량, 환기창면적, 작물유무 모두 내부기온에 유의미한 영향을 미치고 있다. 또한 교호작용도 모두 유의성이 있다는 결과를 보여주고 있다. 다시 말해서 전체적으로 모든 설명변수가 내부기온에 유의미한 영향을 주고 있는 것이다. 그리고 조절된 결정계수의 값이 $R^2_{adj}=0.9273$이므로 이 모형은 데이터의 변동을 약 $92%$ 정도 설명하고 있으므로 설명력도 크다는 것을 알 수 있다. 따라서 더이상의 변수선택과정은 필요없을 것으로 판단되며 이에 따라 최종적인 모형식을 다음과 같이 표현할 수 있겠다.
- ${\hat{y}} = 3.9273 - 0.5799x_{1} + 1.2407x_{2} - 0.4792x_{3} + 0.0133x_{4} - 39.0525x_{5} +9.6095x_{6} + 1.6185x_{1}x_{6} - 0.2081x_{2}x_{6} - 2.4613x_{3}x_{6} - 0.0015x_{4}x_{6} - 52.1623x_{5}x_{6}$
- $y$ : 내부기온, $x _{1}$ : 환기율, $x _{2}$ : 외부기온, $x_{3}$ : 외부풍속, $x_{4}$ : 외부일사량, $x _{5}$ : 환기창면적, $x_{6}$ : 작물유무yes
...
RMSE <- mean(sum((ex1$내부기온-y_hat)^2))
RMSE
```

- 결과적으로 다중선형회귀모형을 적합했을 때 산출되는 RMSE는 $RMSE=3850.218$이다.



## ANN Model

- ANN 모형의 경우는 다음과 같이 적합할 수 있다. hidden node의 경우는 개수를 결정하는 기준점이 따로 정해져있지 않기 때문에 최적의 hidden node의 개수를 미리 정한다는 것은 매우 어려운 일이다. 이에 따라 우선적으로 hidden layer의 개수는 2, 그리고 각각의 layer에 대한 node의 수는 5로 설정한다. 
...
### hidden layer의 개수 2, 각각의 layer에 대한 node의 수 5

```{r}
set.seed(1234)
ann1 <- neuralnet(내부기온~환기율+외부기온+외부풍속+외부일사량+환기창면적+작물유무수치, data=ex1, hidden=c(5,5), threshold = 0.01, stepmax = 1e+10)
summary(ann1)
```
...
RMSE <- mean(sum((ex1$내부기온-y_hat_ann1)^2))
RMSE
```
