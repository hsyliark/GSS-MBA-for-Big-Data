...
- 적합모형은 hidden layer 10개, 각 layer에 대한 node 5개인 모형이다.
- activation function은 `logistic`으로 설정하였다.

```{r}
plantno_scale <- transform(plantno[,c(2,12,1,10,11,13)], 
...
# ex1_train <-as.data.frame(plantno[train.index,])
# ex1_test <- as.data.frame(plantno[-train.index,])
set.seed(1234)
ann1 <- neuralnet(내부기온~외부기온+외부풍속+외부일사량+환기창면적, data=ex1_train, hidden=rep(5,10), threshold = 0.01, stepmax = 1e+08)
summary(ann1)
y_hat_ann1 <- compute(ann1, ex1_test)$net.result
RMSE <- mean(sum(((ex1_test$내부기온-y_hat_ann1)*(max(plantno$내부기온)-min(plantno$내부기온))+min(plantno$내부기온))^2))
RMSE
```

- 적합결과 test RMSE의 값이 111983.6으로 산출되었다. 이는 다중회귀모형과 비교했을 때 좋지 않은 결과인데, 이미 구축한 다중회귀모형이 데이터를 충분히 잘 설명하도록 만들어졌기 때문이라고 생각한다. 
...
```{r}
plantyes_scale <- transform(plantyes[,c(2,12,1,10,11,13)],                                                                           내부기온=(내부기온-min(내부기온))/(max(내부기온)-min(내부기온)),
                           환기율=(환기율-min(환기율))/(max(환기율)-min(환기율)),
                           외부기온=(외부기온-min(외부기온))/(max(외부기온)-min(외부기온)),
                           외부풍속=(외부풍속-min(외부풍속))/(max(외부풍속)-min(외부풍속)),
                           외부일사량=(외부일사량-min(외부일사량))/(max(외부일사량)-min(외부일사량)),
...
# ex1_train <- as.data.frame(plantyes[train.index,])
# ex1_test <- as.data.frame(plantyes[-train.index,])
set.seed(1234)
ann1 <- neuralnet(내부기온~외부기온+외부풍속+외부일사량+환기창면적, data=ex1_train, hidden=rep(5,10), threshold = 0.01, stepmax = 1e+08)
summary(ann1)
y_hat_ann1 <- compute(ann1, ex1_test)$net.result
RMSE <- mean(sum(((ex1_test$내부기온-y_hat_ann1)*(max(plantyes$내부기온)-min(plantyes$내부기온))+min(plantyes$내부기온))^2))
RMSE
```

- 적합결과 test RMSE의 값이 128020.4로 산출되었다. 이는 다중회귀모형과 비교했을 때 좋지 않은 결과인데, 이미 구축한 다중회귀모형이 데이터를 충분히 잘 설명하도록 만들어졌기 때문이라고 생각한다. 
