...
```{r}
ex1 <- plantno[,c(3,12,1,10,11,13,14,15)]
head(ex1)
ex2 <- ex1[,1:6]
```
...
- 산점도를 통해 알 수 있는 사실은 환기율과 환기창면적이 증가할수록, 외부기온과 외부일사량이 떨어질 수록 내외부기온차가 내려간다는 점이다. 환기창면적과의 교호작용은 뚜렷하게 관측되는 것이 없으나, 외부풍속과의 교호작용의 존재 가능성이 추측된다. 외부풍속은 약하지만 내외부기온차와 음의 상관을 보이는 것으로 생각된다.
...
```{r}
m_no <- lm(내외부기온차~환기율+외부기온+외부풍속+외부일사량+환기창면적+환기창면적*환기율+환기창면적*외부기온+환기창면적*외부풍속+환기창면적*외부일사량,data=plantno)
summary(m_no)
```
...
- 조정된 결정계수의 값이 $R^2_{adj}=0.9363$이다.

- 외부풍속과 환기창면적을 제외한 후 모형을 적합해 보면 다음과 같다. 교호작용은 환기율을 중심으로 형성되도록 하였다.

```{r}
m_no <- lm(내외부기온차~환기율+외부기온+외부일사량+환기율*외부기온+환기율*외부일사량,data=plantno)
summary(m_no)
```

- 모든 변수가 유의하고 조정된 결정계수의 값이 $R^2_{adj}=0.8898$이다.
...
```{r}
library(leaps)
ms0 <- regsubsets(내외부기온차~외부기온+외부풍속+외부일사량+환기창면적+환기창면적*외부기온+환기창면적*외부풍속+환기창면적*외부일사량,data=plantno,
                     nvmax=7)
...
```{r}
library(leaps)
ms0 <- regsubsets(내외부기온차~외부기온+외부풍속+외부일사량,data=plantno,
                     nvmax=3)
ms0.summary <- summary(ms0)
ms0.summary
...
m0 <- lm(내외부기온차~환기율+외부기온+외부풍속+외부일사량+환기창면적*환기율+환기창면적+환기창면적*외부기온+환기창면적*외부풍속+환기창면적*외부일사량,data=ex1_train) 
y_hat0 <- predict(m0,ex1_test)
RMSE0 <- mean(sum((ex1_test$내부기온-y_hat0)^2))
RMSE0

# Adj R^2, Cp, BIC 등의 기준을 바탕으로 선택된 모형 (교호작용이 있는 경우는 주효과 포함)
m0 <- lm(내외부기온차~외부기온+외부풍속+외부일사량,data=ex1_train) 
y_hat0 <- predict(m0,ex1_test)
...
- 결과적으로 3가지 모형들 중 모든 주효과와 가장 중요한 교호작용 1가지를 포함한 모형이 가장 우수하였다.
...
```{r}
ex1 <- plantyes[,c(3,12,1,10,11,13,14,15)]
head(ex1)
ex2 <- ex1[,1:6]
```
...
```{r}
m_yes <- lm(내외부기온차~환기율+외부기온+외부풍속+외부일사량+환기창면적+환기창면적*환기율+환기창면적*외부기온+환기창면적*외부풍속+환기창면적*외부일사량,data=plantyes)
summary(m_yes)
```

- 모형적합 결과 외부풍속이 유의수준 0.05에서 유의미하지 않았다. 하지만 외부풍속과 환기창면적 사이의 교호작용이 유의하기 때문에 굳이 제외할 필요는 없다고 판단된다. 조정된 결정계수의 값이 $R^2_{adj}=0.8959$이므로 작물이 없을 때와 비교하면 약간 낮은 값이다.
...
```{r}
library(leaps)
ms0 <- regsubsets(내외부기온차~외부기온+외부풍속+외부일사량+환기창면적+환기창면적*외부기온+환기창면적*외부풍속+환기창면적*외부일사량,data=plantyes,
                     nvmax=7)
ms0.summary <- summary(ms0)
ms0.summary
...
- 분석결과 $R_{adj}^2$의 경우는 변수의 개수 7인 모형, $Cp$의 경우는 변수의 개수 4인 모형, 그리고 $BIC$의 경우는 변수의 개수 3인 모형을 최적으로 판단하고 있다. 이들 중 가장 단순한 모형인 변수의 개수 3인 모형을 최종적으로 선택하도록 한다. 복잡한 모형이 항상 바람직한 결과를 주지는 않으며 단순한 모형도 설명력이 높은 경우가 많기 때문이다.
...
```{r}
# 모든 주효과, 그리고 환기창면적과 다른 변수들 사이의 교호작용을 포함한 모형
m0 <- lm(내외부기온차~환기율+외부기온+외부풍속+외부일사량+환기창면적+환기창면적*환기율+환기창면적*외부기온+환기창면적*외부풍속+환기창면적*외부일사량,data=ex1_train) 
y_hat0 <- predict(m0,ex1_test)
RMSE0 <- mean(sum((ex1_test$내부기온-y_hat0)^2))
RMSE0

# Adj R^2, Cp, BIC 등의 기준을 통해 선택된 모형
m0 <- lm(내외부기온차~외부풍속+외부일사량+환기창면적+환기창면적*외부일사량,data=ex1_train) 
y_hat0 <- predict(m0,ex1_test)
RMSE1 <- mean(sum((ex1_test$내부기온-y_hat0)^2))
RMSE1
...
```{r}
summary(lm(내외부기온차~환기율+외부기온+외부풍속+외부일사량+환기창면적+환기창면적*환기율+환기창면적*외부기온+환기창면적*외부풍속+환기창면적*외부일사량,data=ex1_train))
```
...
- 따라서 내외부기온차를 반응변수로 하여 각 변수의 주효과와 환기율과 외부일사량, 그리고 환기창면적과 외부일사량 사이의 교호작용을 포함한 모형을 시작점으로 하여 분석을 진행하도록 한다. 여기에 주효과와 작물유무 사이의 교호작용도 우선 포함한다. 

```{r}
m_total <- lm(내외부기온차~환기율+외부기온+외부풍속+외부일사량+환기창면적+작물유무+환기율*작물유무+외부기온*작물유무+외부풍속*작물유무+외부일사량*작물유무+환기창면적*작물유무+외부일사량*환기율+환기창면적*외부일사량,data=plant)
summary(m_total)
```
...
- 조정된 결정계수의 값이 $R^2_{adj}=0.8999$이다.
...
```{r}
library(leaps)
ms0 <- regsubsets(내외부기온차~외부기온+외부풍속+외부일사량+환기창면적+작물유무+외부기온*작물유무+외부풍속*작물유무+외부일사량*작물유무+환기창면적*작물유무+환기창면적*외부일사량,data=plant,nvmax=10)
ms0.summary <- summary(ms0)
ms0.summary
...
```{r}
library(leaps)
ms0 <- regsubsets(내외부기온차~외부기온+외부풍속+외부일사량+작물유무+외부기온*작물유무+외부풍속*작물유무+외부일사량*작물유무,data=plant,nvmax=7)
ms0.summary <- summary(ms0)
ms0.summary
...
```{r}
summary(lm(내외부기온차~외부기온+외부풍속+외부일사량+작물유무+외부기온*작물유무,data=plant))
```
...
```{r}
# 최초로 고려한 모형
m0 <- lm(내외부기온차~환기율+외부기온+외부풍속+외부일사량+환기창면적+작물유무+환기율*작물유무+외부기온*작물유무+외부풍속*작물유무+외부일사량*작물유무+환기창면적*작물유무+외부일사량*환기율+환기창면적*외부일사량,data=ex1_train)
y_hat0 <- predict(m0,ex1_test)
RMSE0 <- mean(sum((ex1_test$내부기온-y_hat0)^2))
RMSE0

# Adj R^2, Cp, BIC 등의 기준을 바탕으로 선택된 모형 (교호작용이 있는 경우는 주효과 포함)
m0 <- lm(내외부기온차~외부기온+외부풍속+외부일사량+작물유무+외부기온*작물유무,data=plant)
y_hat0 <- predict(m0,ex1_test)
RMSE1 <- mean(sum((ex1_test$내부기온-y_hat0)^2))
RMSE1
...
m0 <- lm(내외부기온차~외부기온+외부풍속+외부일사량+환기창면적+작물유무+외부풍속*작물유무+환기창면적*외부일사량,data=ex1_train)
y_hat0 <- predict(m0,ex1_test)
RMSE2 <- mean(sum((ex1_test$내부기온-y_hat0)^2))
RMSE2
```

- 결과적으로 3가지 모형들 중 모든 주효과와 가장 중요한 교호작용 2가지를 포함한 모형이 가장 우수하였다.

```{r}
summary(lm(내외부기온차~외부기온+외부풍속+외부일사량+환기창면적+작물유무+외부풍속*작물유무+환기창면적*외부일사량,data=ex1_train))
```
...
- 적합모형은 hidden layer 1개, layer에 대한 node 3개인 모형이다.
...
ex1_test <- plantno[-train.index,]
set.seed(1234)
ann1 <- neuralnet(내외부기온차~환기율+외부기온+외부풍속+외부일사량+환기창면적, data=ex1_train, hidden=3, threshold = 0.01, stepmax = 1e+08)
summary(ann1)
