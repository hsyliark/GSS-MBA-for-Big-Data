...
```{r}
plantno_scale <- transform(plantno[,c(3,12,1,10,11,13)], 
...
set.seed(1234)
ann1 <- neuralnet(내외부기온차~외부기온+외부풍속+외부일사량+환기창면적, data=ex1_train, hidden=rep(5,10), threshold = 0.01, stepmax = 1e+08)
summary(ann1)
y_hat_ann1 <- compute(ann1, ex1_test)$net.result
RMSE <- mean(sum(((ex1_test$내외부기온차-y_hat_ann1)*(max(plantno$내외부기온차)-min(plantno$내외부기온차))+min(plantno$내외부기온차))^2))
RMSE
```

- 적합결과 test RMSE의 값이 1283.314로 산출되었다.
- 다중회귀모형의 결과와 비교했을 때 확실히 예측력 측면에서 성능이 좋은 ANN 모형이 구축되었다고 볼 수 있다.
...
```{r}
plantyes_scale <- transform(plantyes[,c(3,12,1,10,11,13)],    
                           내외부기온차=(내외부기온차-min(내외부기온차))/(max(내외부기온차)-min(내외부기온차)),
                           환기율=(환기율-min(환기율))/(max(환기율)-min(환기율)),
                           외부기온=(외부기온-min(외부기온))/(max(외부기온)-min(외부기온)),
                           외부풍속=(외부풍속-min(외부풍속))/(max(외부풍속)-min(외부풍속)),
...
summary(ann1)
y_hat_ann1 <- compute(ann1, ex1_test)$net.result
RMSE <- mean(sum(((ex1_test$내외부기온차-y_hat_ann1)*(max(plantyes$내외부기온차)-min(plantyes$내외부기온차))+min(plantyes$내외부기온차))^2))
RMSE
```

- 적합결과 test RMSE의 값이 558.834로 산출되었다. 
- 다중회귀모형의 결과와 비교했을 때 확실히 예측력 측면에서 성능이 좋은 ANN 모형이 구축되었다고 볼 수 있다.
