...
- Gamma 분포를 가정한 결과 shape 모수는 1, rate 모수는 0.7897048(scale 모수는 1.266296)로 추정되었다.
- 그리고 Chi-square p-value를 통해 변수 time에 대한 적절한 분포가 Gamma 분포라는 귀무가설을 기각하지 못하므로 Gamma 분포를 가정하는 것이 통계적으로 타당하다고 볼 수 있다.
- 이를 바탕으로 추정된 g(x)를 Gamma(1,1.266296)로 두고 적절한 대체분포 h(x)를 찾아보도록 한다.

```{r}
x <- seq(0,20,0.001)
g <- dgamma(x,shape=f1$cor[1,1],scale=1/f1$cor[2,1])
h <- dgamma(x,shape=1,scale=2)
plot(x,g,ylab="g(x) or h(x)",xlab="x",ylim=c(0,1),type="l",col="#228B22")
par(new=T)
plot(x,h,ylab="",xlab="x",ylim=c(0,1),type="l",col="#FF3030")
legend(x=5,y=1, c("g(x) : Gamma(1,1.266296)","h(x) : Gamma(1,2)"),lty=1,col=c("#228B22","#FF3030"))
```

- h(x)를 g(x)에 비해 오른쪽 꼬리가 더 두꺼운 분포로 선정하였다.
- h(x) : Gamma(1,2)
- 모의실험은 Case1,2 에서 진행한 방식을 동일하게 적용한다.
...
ISB <- c()
for (i in 1:1000) {
  gr <- rgamma(1000,shape=f1$cor[1,1],scale=1/f1$cor[2,1])
  GM[i] <- mean(gr)
  GV[i] <- var(gr)
  GB[i] <- abs(mean(gr)-f1$cor[1,1]/f1$cor[2,1])
  v <- rgamma(1000,shape=1,scale=2)
  g <- dgamma(v,shape=f1$cor[1,1],scale=1/f1$cor[2,1])
  h <- dgamma(v,shape=1,scale=2) 
  ISM[i] <- mean(v*g/h)
  ISV[i] <- var(v*g/h)
  ISB[i] <- abs(mean(v*g/h)-f1$cor[1,1]/f1$cor[2,1])
}
...
```{r}
(MSE.GM <- mean((res3$mean[res3$method=="Sampling"]-f1$cor[1,1]/f1$cor[2,1])^2))
(MSE.ISM <- mean((res3$mean[res3$method=="Importance Sampling"]-f1$cor[1,1]/f1$cor[2,1])^2))
MSE.GM > MSE.ISM
...
Var.GM > Var.ISM
(Bias2.GM <- (mean(res3$mean[res3$method=="Sampling"])-f1$cor[1,1]/f1$cor[2,1])^2)
(Bias2.ISM <- (mean(res3$mean[res3$method=="Importance Sampling"])-f1$cor[1,1]/f1$cor[2,1])^2)
Bias2.GM > Bias2.ISM
```
...
- Weibull 분포를 가정한 결과 shape 모수는 1, scale 모수는 0.3102541로 추정되었다.
- 그리고 Chi-square p-value를 통해 변수 time에 대한 적절한 분포가 Weibull 분포라는 귀무가설을 기각하지 못하므로 Weibull 분포를 가정하는 것이 통계적으로 타당하다고 볼 수 있다.
- 이를 바탕으로 추정된 g(x)를 Weibull(1,0.3102541)로 두고 적절한 대체분포 h(x)를 찾아보도록 한다.

```{r}
x <- seq(0,20,0.001)
g <- dweibull(x,shape=f2$cor[1,1],scale=f2$cor[2,1])
h <- dweibull(x,shape=1,scale=2)
plot(x,g,ylab="g(x) or h(x)",xlab="x",ylim=c(0,1),type="l",col="#228B22")
par(new=T)
plot(x,h,ylab="",xlab="x",ylim=c(0,1),type="l",col="#FF3030")
legend(x=5,y=1, c("g(x) : Gamma(1,1.266296)","h(x) : Gamma(1,2)"),lty=1,col=c("#228B22","#FF3030"))
...
ISB <- c()
for (i in 1:1000) {
  gr <- rweibull(1000,shape=f2$cor[1,1],scale=f2$cor[2,1])
  GM[i] <- mean(gr)
  GV[i] <- var(gr)
  GB[i] <- abs(mean(gr)-f2$cor[2,1]*gamma(1+1/f2$cor[1,1]))
  v <- rweibull(1000,shape=1,scale=2)
  g <- dweibull(v,shape=f2$cor[1,1],scale=f2$cor[2,1])
  h <- dweibull(v,shape=1,scale=2) 
  ISM[i] <- mean(v*g/h)
  ISV[i] <- var(v*g/h)
  ISB[i] <- abs(mean(v*g/h)-f2$cor[2,1]*gamma(1+1/f2$cor[1,1]))
}
...
```{r}
(MSE.GM <- mean((res4$mean[res4$method=="Sampling"]-f2$cor[2,1]*gamma(1+1/f2$cor[1,1]))^2))
(MSE.ISM <- mean((res4$mean[res4$method=="Importance Sampling"]-f2$cor[2,1]*gamma(1+1/f2$cor[1,1]))^2))
MSE.GM > MSE.ISM
...
Var.GM > Var.ISM
(Bias2.GM <- (mean(res4$mean[res4$method=="Sampling"])-f2$cor[2,1]*gamma(1+1/f2$cor[1,1]))^2)
(Bias2.ISM <- (mean(res4$mean[res4$method=="Importance Sampling"])-f2$cor[2,1]*gamma(1+1/f2$cor[1,1]))^2)
Bias2.GM > Bias2.ISM
```
