...
my.gradient.descent.U <- function(So, X, alpha, lambda, time.sort) {
  ...
  return(list(beta.new=beta.new, iteration=iteration, difference=difference,
              c_index=c_index, sum_C_tilde_t=sum_C_tilde_t))
...
res1 <- my.gradient.descent.U(So, X, alpha=0.2, lambda=0, time.sort3)
res1$beta.new
risk.score_01 <- exp(X%*%res1$beta.new)
res2 <- my.gradient.descent.U(So, X, alpha=0.2, lambda=3, time.sort3)
res2$beta.new
risk.score_02 <- exp(X%*%res2$beta.new)
res3 <- my.gradient.descent.U(So, X, alpha=0.2, lambda=5, time.sort3)
res3$beta.new
risk.score_03 <- exp(X%*%res3$beta.new)

# change alpha
risk.score_cox <- predict(object=coxph(So ~ X), newdata=dt5, type="risk")
res1 <- my.gradient.descent.U(So, X, alpha=0.01, lambda=3, time.sort3)
res1$beta.new
risk.score_01 <- exp(X%*%res1$beta.new)
res2 <- my.gradient.descent.U(So, X, alpha=0.1, lambda=3, time.sort3)
res2$beta.new
risk.score_02 <- exp(X%*%res2$beta.new)
res3 <- my.gradient.descent.U(So, X, alpha=0.3, lambda=3, time.sort3)
res3$beta.new
risk.score_03 <- exp(X%*%res3$beta.new)

# change sorting time point
risk.score_cox <- predict(object=coxph(So ~ X), newdata=dt5, type="risk")
...
res5 <- my.gradient.descent.U(So, X, alpha=0.2, lambda=3, time.sort4)
res5$beta.new
risk.score_05 <- exp(X%*%res5$beta.new)

library(ggplot2)
dat <- data.frame(iteration=res5$iteration, difference=res5$difference,
                  c_index=res5$c_index, sum_C_tilde_t=res5$sum_C_tilde_t)
...
ggplot(dt2_5, aes(x=time, y=AUC, group=case, color=case)) +
  geom_line() +
...
dt2_2 <- dt2[dt2$case=="case00" | dt2$case=="case02",]
dt2_3 <- dt2[dt2$case=="case00" | dt2$case=="case03",]
dt2_4 <- dt2[dt2$case=="case00" | dt2$case=="case04",]
dt2_5 <- dt2[dt2$case=="case00" | dt2$case=="case05",]
