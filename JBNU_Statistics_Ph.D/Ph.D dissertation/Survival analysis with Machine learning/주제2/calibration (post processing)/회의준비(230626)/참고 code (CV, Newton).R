...
dat.gen <- function(n, beta, seed=NULL){
  ...
  dat <- data.frame(y=y, x1=x1, x2=x2, x3=x3, x4=x4, x5=x5,
                    x6=x6, x7=x7, x8=x8, x9=x9, linpred=linpred)
  return( dat )
}
...
KRLRC.miss <- c(rep(0,50))

for (i in 1:50) {
  
  # Making simulation data
  
  beta <- c(1, 0.5^1, 0.5^2, 0.5^3, 0.5^4, 0.5^5, 0.5^6, 0.5^7, 0.5^8, 0.5^9)
  train.sim <- dat.gen(50, beta)
  test.sim <- dat.gen(1000, beta)
  
  train.sim <- train.sim[,-11] ; test.sim <- test.sim[,-11]
  ...
  KRLRC.miss[i] <- h2$misclassification.rate
...
for (i in 1:50) {
  ...
  KRLRCS.miss[i] <- h2$misclassification.rate
...
for (s in 1:50) {
  ...
  KRLRCB.miss1[s] <- mean(y.hat.bag != y.test)
  
}

KRLRCB.miss1
boxplot(KRLRCB.miss1)



## (2) Using logit.hat

KRLRCB.miss2 <- c(rep(0,50))
...
for (s in 1:50) {
  ...
  KRLRCB.miss2[s] <- mean(y.hat.bag != y.test)
  
}

KRLRCB.miss2
boxplot(KRLRCB.miss2)



## (3) Using mean value of test misclassification rate

KRLRCB.miss3 <- c(rep(0,50))
...
for (s in 1:50) {
  ...
  KRLRCB.miss3[s] <- mean(boots.rate.miss)
  
}

KRLRCB.miss3
boxplot(KRLRCB.miss3)
...
for (s in 1:50) {
  ...
  train.sim.X <- train.sim[,2:10] ; test.sim.X <- test.sim[,2:10]
  ...
  KRLRCR.miss1[s] <- mean(y.hat.rf != y.test)
  
}

KRLRCR.miss1
boxplot(KRLRCR.miss1)



## (2) Using logit.hat

KRLRCR.miss2 <- c(rep(0,50))  
...
for (s in 1:50) {
  ...
  KRLRCR.miss2[s] <- mean(y.hat.rf != y.test)
  
}

KRLRCR.miss2
boxplot(KRLRCR.miss2)



## (3) Using mean value of test misclassification rate

KRLRCR.miss3 <- c(rep(0,50))  
...
for (s in 1:50) {
  ...
  KRLRCR.miss3[s] <- mean(boots.rate.miss)
  
}

KRLRCR.miss3
boxplot(KRLRCR.miss3)
...
dat.res <- cbind(KRLRC.miss, KRLRCS.miss, KRLRCB.miss1, KRLRCB.miss2, KRLRCB.miss3,
                 KRLRCR.miss1, KRLRCR.miss2, KRLRCR.miss3)
res.mean <- apply(dat.res, 2, mean)
res.sd <- apply(dat.res, 2, sd)
res.table <- rbind(res.mean, res.sd)
boxplot(dat.res, main="Result of simulation (n=50, p=9)",
        ylab="Test misclassification rate", col=c("red","blue","yellow","green","orange","brown"))
