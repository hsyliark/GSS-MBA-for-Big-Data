---
title: "Time dependent AUC with real data"
...
```{r}
dt5 <- na.omit(pbc)
cox.dep.dt5 <- coxph(Surv(time,status) ~ bili+albumin+protime+edema+age, data=dt5)
summary(cox.dep.dt5)
```
...
ggplot(data=dt6)+
  geom_line(mapping=aes(x=FP,
                        ...
                        color=times),
            size=1) +
  ggtitle("Time dependent ROC curve (pbc data)") +
...
ggplot() +
  ...
  ggtitle("Time dependent AUC with risk score (pbc data)") +
  xlab("time") + ylab("AUC") +
  theme(plot.title = element_text(hjust = 0.5))
```
