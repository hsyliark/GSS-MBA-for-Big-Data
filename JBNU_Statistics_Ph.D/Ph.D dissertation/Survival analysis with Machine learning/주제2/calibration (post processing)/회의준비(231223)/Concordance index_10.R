...
model1 <- coxph(So ~ X)
beta_hat <- model1[["coefficients"]]
time.sort0 <- seq(from=5, to=250, length.out=50) # sorting survival time
...
res1 <- my.gradient.descent.U(So, X, alpha=0.2, lambda=0, time.sort2)
res1$beta.new
risk.score_01 <- exp(X%*%res1$beta.new)
res2 <- my.gradient.descent.U(So, X, alpha=0.2, lambda=3, time.sort2)
res2$beta.new
risk.score_02 <- exp(X%*%res2$beta.new)
res3 <- my.gradient.descent.U(So, X, alpha=0.2, lambda=10, time.sort2)
res3$beta.new
risk.score_03 <- exp(X%*%res3$beta.new)

risk.score_cox <- predict(object=coxph(So ~ X), newdata=dt5, type="risk")
res1 <- my.gradient.descent.U(So, X, alpha=0.01, lambda=5, time.sort2)
res1$beta.new
risk.score_01 <- exp(X%*%res1$beta.new)
res2 <- my.gradient.descent.U(So, X, alpha=0.1, lambda=5, time.sort2)
res2$beta.new
risk.score_02 <- exp(X%*%res2$beta.new)
res3 <- my.gradient.descent.U(So, X, alpha=0.5, lambda=5, time.sort2)
res3$beta.new
risk.score_03 <- exp(X%*%res3$beta.new)
