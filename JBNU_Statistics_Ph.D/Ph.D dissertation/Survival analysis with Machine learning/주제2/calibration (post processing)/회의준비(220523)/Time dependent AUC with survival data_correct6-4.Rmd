...
mean(1-censor) # censoring rate
dt7 <- data.frame(X=X, Y=Y, C=C, M=M, MZ=MZ, time=t, censor=censor)
head(dt7,10)
...
```{r}
cox.dep.dt7 <- coxph(Surv(time,censor) ~ X, data=dt7)
# cox.dep.dt7 <- gam(time ~ X + s(time,by=M), weights=censor, data=dt7, family=cox.ph) ???
summary(cox.dep.dt7)
risk.dep.dt7 <- predict(object=cox.dep.dt7, newdata=dt7, type="risk") # risk score
...
ggplot() +
  ...
  geom_vline(xintercept = min(dt7$time[dt7$censor==1]),
             linetype="dotted", color = "blue", size=1) +
  ggtitle("Time dependent AUC with risk score (cox.dep.dt7)") +
  xlab("time") + ylab("AUC") +
  theme(plot.title = element_text(hjust = 0.5))
```
...
for (i in 1:100) {
  ...
  for (j in 1:200) {
    ...
    U2[j] <- runif(1)
    X[j] <- rnorm(1,mean=0,sd=1)
    M[j] <- runif(1,min=0,max=1)
    Y[j] <- (1/gamma0*M[j]*alpha0)*log(1-(gamma0*M[j]*alpha0/lambda1/exp(beta0*X[j]))*log(U1[j]))
    MZ[j] <- M[j]*Y[j]
    C[j] <- (lambda2)*(1-U2[j])
    t[j] <- min(Y[j],C[j])
    censor[j] <- ifelse(Y[j]<=C[j],1,0)
  ...
  con.AUC[,i] <- ROC.risk.dep.dt7[["AUC"]]
  
  # print(i)
  
}

dat7 <- data.frame(time=seq(0,25,0.01), con.AUC=rowMeans(con.AUC, na.rm=TRUE))
plot(x=dat7$time, y=dat7$con.AUC, xlab="time", ylab="mean of AUC",
     main="Continuous time-varying covariate \n (Exponential distribution of event times)", type="l", col="red", ylim=c(0.7,1))
```
