---
title: "Time dependent AUC with survival data _ correct6-3"
...
- $z(t)=\alpha t$
- $h_{0}(t)=\lambda$
- True coefficients : $\beta=-3$, $\gamma=2$, $\alpha=4$
...
ggplot() +
  ...
  ggtitle("Time dependent AUC with risk score (cox.dep.dt5), beta=-3") +
  xlab("time") + ylab("AUC") +
...
```{r}
con.AUC <- matrix(nrow=length(seq(0,30,0.01)),ncol=100)
...
dat5 <- data.frame(time=seq(0,30,0.01), con.AUC=rowMeans(con.AUC, na.rm=TRUE))
plot(x=dat5$time, y=dat5$con.AUC, xlab="time", ylab="mean of AUC",
     main="Continuous time-varying covariate \n (Exponential distribution of event times), beta=-3", type="l", col="red", ylim=c(0.5,1))
```
...
```{r}
con.AUC <- matrix(nrow=length(seq(0,25,0.01)),ncol=100)
...
dat5 <- data.frame(time=seq(0,25,0.01), con.AUC=rowMeans(con.AUC, na.rm=TRUE))
plot(x=dat5$time, y=dat5$con.AUC, xlab="time", ylab="mean of AUC",
     main="Continuous time-varying covariate \n (Exponential distribution of event times), beta=3", type="l", col="red", ylim=c(0.5,1))
```
...
```{r}
lambda1 <- 0.003 # parameter for generate Y  
lambda2 <- 2.5/lambda1 # parameter for generate C 
...
- $h_{0}(t)=\lambda$
- True coefficients : $\beta=-3$, $\gamma=2$
...
ggplot() +
  ...
  geom_vline(xintercept = min(dt6$time[dt6$censor==1]),
             linetype="dotted", color = "blue", size=1) +
  ggtitle("Time dependent AUC with risk score (cox.dep.dt6), beta=-3") +
  xlab("time") + ylab("AUC") +
  theme(plot.title = element_text(hjust = 0.5))
```
...
```{r}
din.AUC <- matrix(nrow=length(seq(0,900,0.1)),ncol=100)
...
dat6 <- data.frame(time=seq(0,900,0.1), din.AUC=rowMeans(din.AUC, na.rm=TRUE))
plot(x=dat6$time, y=dat6$din.AUC, xlab="time", ylab="mean of AUC",
     main="Dichotomous time-varying covariate \n (Exponential distribution of event times), beta=-3", type="l", col="blue", ylim=c(0.7,1))
```
...
```{r}
din.AUC <- matrix(nrow=length(seq(0,80,0.01)),ncol=100)
...
dat6 <- data.frame(time=seq(0,80,0.01), din.AUC=rowMeans(din.AUC, na.rm=TRUE))
plot(x=dat6$time, y=dat6$din.AUC, xlab="time", ylab="mean of AUC",
     main="Dichotomous time-varying covariate \n (Exponential distribution of event times), beta=3", type="l", col="blue", ylim=c(0.6,1))
```
...
## Case 3) Continuous time-varying covariate: exponential distribution of event times

- $Y$ : real survival time

- $C$ : censoring time  

- model : $h(t,x,m,z(t))=h_{0}(t)exp(\beta x + \gamma mz(t))$
- $x$ : independent variable, $m$ : interaction with time dependent term 
- $Y$ : Assume $z(t)=\alpha t$, $\alpha > 0$
- $h_{0}(t)=\lambda$
- $h(t,x,m,z(t))=\lambda exp(\beta x + \gamma m \alpha t)=-\frac{d}{dt} log[S(t,x,m,z(t))]$
- $log[S(t,x,m,z(t))]=-\int_{0}^{t}\lambda exp(\beta x + \gamma m \alpha s)ds=-\frac{\lambda exp(\beta x)}{\gamma m \alpha} [exp(\gamma m \alpha t)-1]$
- $S(t,x,m,z(t))=exp[-\frac{\lambda exp(\beta x)}{\gamma m \alpha} [exp(\gamma m \alpha t)-1]]$ 
- $F(t,x,m,z(t))=1-exp[-\frac{\lambda exp(\beta x)}{\gamma m \alpha} [exp(\gamma m \alpha t)-1]]$
- $F^{-1}(t,x,m,z(t))=\frac {1}{\gamma m \alpha} log[1-\frac {\gamma m \alpha}{\lambda exp(\beta x)} log(1-t)]$
- $C$ : $f_{C0}(t)=\frac {1}{\lambda}, 0<t<\lambda$ -> $S_{C0}(t)=1-\frac {t}{\lambda}$ -> $h_{C0}(t)=\frac {1}{\lambda -t}$
- By Inverse Transform Sampling and non-informative censoring, suppose that $U_{1}, U_{2} \overset{i.i.d}{\sim}\ U(0,1)$, then we can generate real survival time $Y$ and censoring time $C$ like this.
- $\therefore Y=\frac {1}{\gamma m \alpha} log[1-\frac {\gamma m \alpha}{\lambda_{1} exp(\beta x)} log(U_{1})]$
- $\therefore C=\lambda_{2}(1-U_{2})$ 
...
t <- c() # observed survival time
censor <- c() # censoring indicator
X <- c() # time invariant covariate : N(0,1^2)
M <- c() # interaction with time dependent term : U(0,1)
U1 <- c() # random variable from U(0,1) for generate Y
U2 <- c() # random variable from U(0,1) for generate C 
...
for (i in 1:1000) {
  ...
  U2[i] <- runif(1)
  X[i] <- runif(1,min=0,max=1)
  M[i] <- runif(1,min=0,max=1)
  Y[i] <- (1/gamma0*M[i]*alpha0)*log(1-(gamma0*M[i]*alpha0/lambda1/exp(beta0*X[i]))*log(U1[i]))
  C[i] <- (lambda2)*(1-U2[i])
  t[i] <- min(Y[i],C[i])
  censor[i] <- ifelse(Y[i]<=C[i],1,0)
}
...
mean(1-censor) # censoring rate
dt7 <- data.frame(X=X, Y=Y, C=C, M=M, time=t, censor=censor)
head(dt7,10)
min(dt7$time[dt7$censor==1])
```
...
- model : $h(t,x,m,z(t))=h_{0}(t)exp[\beta x + \gamma m z(t)]$
- $z(t)=\alpha t$
- $h_{0}(t)=\lambda$
- True coefficients : $\beta=-3$, $\gamma=1$, $\alpha=2$

- https://stat.ethz.ch/R-manual/R-devel/library/mgcv/html/coxph.html

### Set Cox regression model

```{r}
cox.dep.dt7 <- gam(time ~ X + s(time,by=M), weights=censor, data=dt7, family=cox.ph) 
summary(cox.dep.dt7)
risk.dep.dt7 <- predict(object=cox.dep.dt7, newdata=dt7, type="link") # linear predictor (risk score)
dt7$risk.dep.dt7 <- risk.dep.dt7
ROC.risk.dep.dt7 <- timeROC(T=dt7$time,
                         delta=dt7$censor,marker=dt7$risk.dep.dt7,
                         cause=1,weighting="marginal",
                         times=dt7$time)
ROC.risk.dep.dt7
```
...
```{r}
# uncensored
uncensored.density7 <- density(dt7$time[dt7$censor==1])
data7_1 <- data.frame(time=uncensored.density7[["x"]], 
                    density=uncensored.density7[["y"]])
data7_1$density.adjusted <- (max(na.omit(ROC.risk.dep.dt7[["AUC"]]))-min(na.omit(ROC.risk.dep.dt7[["AUC"]])))*(uncensored.density7[["y"]]-min(uncensored.density7[["y"]]))/(max(uncensored.density7[["y"]])-min(uncensored.density7[["y"]]))+min(na.omit(ROC.risk.dep.dt7[["AUC"]]))

# censored
censored.density7 <- density(dt7$time[dt7$censor==0])
data7_2 <- data.frame(time=censored.density7[["x"]], 
                    density=censored.density7[["y"]])
data7_2$density.adjusted <- (max(na.omit(ROC.risk.dep.dt7[["AUC"]]))-min(na.omit(ROC.risk.dep.dt7[["AUC"]])))*(censored.density7[["y"]]-min(censored.density7[["y"]]))/(max(censored.density7[["y"]])-min(censored.density7[["y"]]))+min(na.omit(ROC.risk.dep.dt7[["AUC"]]))

res7 <- data.frame(time=ROC.risk.dep.dt7[["times"]],
                    cox.dep.dt7=ROC.risk.dep.dt7[["AUC"]])
res7 <- melt(data = res7, 
                   id.vars = "time", 
                   measure.vars = c("cox.dep.dt7"))
```

#### Time dependent AUC plot with ggplot2 (cox.dep.dt7)

```{r}
dt7$censor1 <- ifelse(dt7$censor==1,
                    1,min(na.omit(ROC.risk.dep.dt7[["AUC"]])))
ggplot() +
  geom_line(data=res7, aes(x=time, y=value,
                            group=variable, color=variable)) +
  geom_point(data=dt7, aes(x=time, y=censor1, color=as.factor(censor))) +
  geom_line(data=data7_1, aes(x=time, y=density.adjusted), 
            size=1, colour='black') +
  geom_line(data=data7_2, aes(x=time, y=density.adjusted), 
            size=1, colour='orange') +
  scale_x_continuous(limits = c(0, max(dt7$time))) +
  geom_vline(xintercept = min(dt7$time[dt7$censor==1]),
             linetype="dotted", color = "blue", size=1) +
  ggtitle("Time dependent AUC with risk score (cox.dep.dt7), beta=-3") +
  xlab("time") + ylab("AUC") +
  theme(plot.title = element_text(hjust = 0.5))
```
...
#### Mean value of Time dependent AUC (cox.dep.dt7)

```{r}
mean(ROC.risk.dep.dt7[["AUC"]],na.rm=TRUE) 
```


#### simulation (repeat 100 times / time VS mean of AUC)

- simulation(repeat 100 times)은 다음과 같은 오류가 일어나는 관계로 아직 실시하지 않았습니다.

```
Error in eigen(hess1, symmetric = TRUE) : 
'x'에 무한(infinite) 또는 결측(missing)값이 존재합니다.
```
