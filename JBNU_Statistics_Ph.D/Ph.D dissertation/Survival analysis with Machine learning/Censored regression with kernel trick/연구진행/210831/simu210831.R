...
my.ridge.regression <- function(y, X, lambda) {
  ...
  beta.hat <- solve(t(X)%*%X + lambda*diag(x=1, nrow=length(y), ncol=length(y)))%*%t(X)%*%y
...
fit.ridge <- function(y.train, X.train, lambda) {
...
pred.ridge <- function(y.test, X.test, beta.hat) {
...
cv.ridge <- function(y.train, X.train, k, grid.l) {
  ...
  for (j in 1:r) {
    ...
    for (i in 0:(k-1)) {
      ...
      a1 <- fit.ridge(y.sim.train, X.sim.train, lambda[j])
      train.beta.hat <- a1$beta.hat
      
      a2 <- pred.ridge(y.sim.test, X.sim.test, train.beta.hat)
      test.y.hat <- a2$y.hat      
...
fit.ftn1 <- function(dat.sim) {
  ...
  for (i in 1:100) {
    ...
    train.index <- sample(1:nrow(dat.sim), round(nrow(dat.sim)/2), replace=F)
    
    train.sim <- dat.sim[train.index,]
    test.sim <- dat.sim[-train.index,]
    n.train <- round(nrow(dat.sim)/2) 
    ...
    h <- cv.ridge(y.train, X.train, 5, grid.l) 
    ...
    h1 <- fit.ridge(y.train, X.train, best.lam)
    
    # Calculate test RMSE
    
    sim.beta.hat <- h1$beta.hat
    
    h2 <- pred.ridge(y.test, X.test, sim.beta.hat)
    RR[i] <- h2$rmse
...
ggplot(dat.res, aes(x = method, y = RMSE, fill = method)) + geom_boxplot()  
