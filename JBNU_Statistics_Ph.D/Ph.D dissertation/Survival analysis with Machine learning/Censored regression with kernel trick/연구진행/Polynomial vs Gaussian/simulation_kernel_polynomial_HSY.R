...
ggplot(dat.res13_1, aes(x = method, y = RMSE, fill = method)) + geom_boxplot() 
ggplot(dat.res13_2, aes(x = method, y = RMSE, fill = method)) + geom_boxplot()
ggplot(dat.res13_3, aes(x = method, y = RMSE, fill = method)) + geom_boxplot()
# Boxplot for row binded dataset
dat.res13[dat.res13$number==50,]$number <- "1(50)"
dat.res13[dat.res13$number==100,]$number <- "2(100)"
dat.res13[dat.res13$number==200,]$number <- "3(200)"
ggplot(dat.res13, aes(x = number, y = RMSE, fill = number)) + geom_boxplot() +
  facet_wrap(~ method, ncol=16) + theme(axis.text.x=element_text(angle=45, hjust=1))
write.csv(dat.res13, "C:/Users/Hi/Desktop/Polynomial vs Gaussian/p9cen0.csv")
